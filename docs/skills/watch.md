...
- **Inspect the audio.** Use `media_waveform` for changes in level and `media_transcribe` for speech. Local mode has no semantic audio-listening model; do not guess what music or an unidentified sound is from the waveform.
...
The `media_*` tools take a path or a direct media URL. A web page URL is not a media URL; obtain a local copy through a source you are allowed to use before probing it. Frameyard does not install a site downloader.

# Matching depth to the question

Read only as much of the footage as the answer requires.
...
- A waveform can locate sound events but cannot identify them. State that limit if no listening capability is available.
...
- For open-ended questions ("summarize this", "what happens here"), combine structure from waveform and filmstrip, speech from the transcript, and frames at the moments that matter.
