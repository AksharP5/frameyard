...
- **Inspect the audio.** Use `media_waveform` to locate changes in level and `media_transcribe` for spoken words. Frameyard's hosted `media_listen` tool is unavailable in local mode, so do not treat a waveform as evidence of what a non-speech sound is.
...
- Import media into the project library. Use **Collect project media** before sharing a project that references files elsewhere on the machine.
- `context` reports the open project, current scene, playhead, selection, and assets.
...
- Follow the project's style and the user's brief. Keep titles and overlays out when the picture already makes the point.
...
- Use native `<scene3d>` and mesh elements when the result should stay editable in Frameyard. Use `<surface>` for custom rendering that native elements cannot express.
...
- Preserve the project already open unless the user asks for a new one.
...
- [Local transcription](../reference/tools/media/transcribe.md)
