...
| `link` | Nonempty token shared by clips edited together within the same scene. Saved by Link clips; video/audio imports receive a token automatically. |
| `locked` | Protects this clip or container and its descendants from editor changes. Does not change playback or export. |
...
Frame units use the project frame rate, configured by `diffusion.frameRate` in
the project's `package.json`. The default is 30. The editor supports rates from
1 through 240, including fractional rates such as `60000 / 1001`. Numeric seconds
keep their duration when the project rate changes; an authored `"60f"` lasts one
second in a 60 fps project. A different export rate does not reinterpret authored
frame units. The runtime uses frames at its current preview or export rate.

Authored values may be negative. Interactive placement keeps moved clips within
the scene's nonnegative timeline and preserves linked offsets.
...
| `"HH:MM:SS:FF"` | `"00:00:01:15"` | Non-drop timecode at the project rate. Frame field uses the nominal rounded rate. |
