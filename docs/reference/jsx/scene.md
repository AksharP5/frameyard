...
| `workarea` | `[in, out] \| null` | `null` | The stretch of the scene that plays and exports, in any [time format](./timing.md#time-formats). Without one the whole scene plays and exports. |
| `markers` | `{ id: string; time: Time; name: string }[]` | none | Named timeline markers with unique IDs and nonnegative times. |
| `audioProcessing` | `AudioProcessingSettings` | none | Processing on the master audio bus. See [audio.md](./audio.md#processing). |
| `colorGrade` | `ColorGradeSettings` | none | White balance and curves on the scene composite. See [styles.md](./styles.md#color-grading). |
| `selected` | `boolean` | absent | Whether the editor has the scene selected. |
| `depthSort` | `"layer" \| "camera"` | `"layer"` | Child plane ordering: local `z` with stable ties, or camera distance for solid assemblies. |
| `perspective` | `number` | `1000` | Camera focal length in pixels. |
| `cameraX`, `cameraY`, `cameraZ` | `number` | Scene center, center, `1000` | Camera position in scene coordinates. |
| `cameraZoom` | `number` | `1` | Camera magnification. |
| `cameraRotationX`, `cameraRotationY`, `cameraRotation` | `number` | `0` | Camera orientation in degrees. |
| `focusDistance`, `aperture` | `number` | `1000`, `0` | Focus-plane distance and depth-of-field blur strength in pixels. |
| `bloom`, `vignette`, `grain` | `number` | `0` | Finishing strengths, 0–1. |
| `colorSplit` | `number` | `0` | Chromatic separation in pixels. |
| `physics` | `PhysicsWorldOptions \| boolean` | absent | Physics world for descendant rigid bodies; see [native physics](./spatial.md#physics). |
...
Camera and finishing properties accept ordinary [keyframe tracks](./keyframes.md). They affect playback and export; the stage `camera` remains the editor viewport.
