...
- **`AssetRef`**: the value returned by a hosted `generate.*` declaration (see [generate.md](./generate.md)). Hosted generation is unavailable in Frameyard's default local mode.
...
## MOV playback on Linux

MOV files are accepted. A local video whose codec cannot be decoded directly, such as DNxHR, ProRes or unsupported HEVC, gets a compatible playback copy through the installed FFmpeg. Preparation shows progress and runs once per source version. Copies live in `.cache/playback/`, outside project checkpoints. Supported videos continue to use their original bytes.

The Source preview retains dimensions and frame timing, using H.264 at CRF 16 and
AAC audio at 320 kbps. Half and Quarter preview use reduced dimensions and also
apply to directly decodable video. These are lossy preview copies. Cache keys
include the source version and preview scale.

Final renders read original video and audio. Browser-decodable originals use
WebCodecs; other local SDR video uses native FFmpeg frame decoding. Selected audio
streams use a lossless PCM cache when required. Preview copies do not supply final
render frames. The compositor is 8-bit SDR, so this does not preserve HDR or a
high-bit-depth mastering pipeline. Variable-rate native sources are sampled on
their reported frame-rate grid. Unsupported alpha preview and HDR sources report
an error; use a PNG sequence for transparent preview or an SDR source for HDR.

The original file, asset identity, saved source path and timeline edits remain
unchanged. **Save as** and **Download all assets** copy original bytes.
...
A sequence's frame rate determines its duration: 600 frames at 24 fps is 25 seconds, at 60 fps is 10. Retained animation renders include `.sequence.json` metadata with their original frame rate. Other sequences default to 30 fps. The optional `frameRate` prop overrides either default; it does not affect encoded video or still images.

```tsx
<video src="renders/beauty-pass" frameRate={24} width={1920} height={1080} />
```

`frameRate` is not [`playbackRate`](./timing.md#playback-rate), which retimes a source against the timeline whatever its natural speed is; this is what that natural speed *is*. Project and export frame rates are independent settings.

## Transforms

Hosted builds can put a source through a model before the element shows it, by
declaring the result as the `src`. These transforms are unavailable in default
local mode. See [generated assets](./generate.md#transforms):
