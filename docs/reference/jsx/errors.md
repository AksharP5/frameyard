...
4. **`Error drawing <HtmlPaint> content: … No cached paint record for element`** in `logs`, or HTML missing from early captured frames while later frames render: retry the affected time after the host mounts. If it repeats, include the log entry in a [bug report](../tools/report.md).
