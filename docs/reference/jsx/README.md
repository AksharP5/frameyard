...
| [group.md](./group.md), [rect.md](./rect.md), [path.md](./path.md), [text.md](./text.md), [video.md](./video.md), [image.md](./image.md), [audio.md](./audio.md) | Per-element props |
...
| [motion-workspace.md](./motion-workspace.md) | Motion library, inspector controls and editable animation conversion |
| [spatial.md](./spatial.md) | Native 3D viewports, meshes, XYZ paths, points, lights, volumes, physics and agent authoring |
...
| [preset.md](./preset.md) | `<preset>`: editable motion graphics and live scene effects |
| [highlight.md](./highlight.md) | `<highlight>`: enlarge and center a live region with background blur and dimming |
| [audio-sync.md](./audio-sync.md) | `syncTo` audio alignment |
| [captions.md](./captions.md) | `<captions>` and style presets |
| [generate.md](./generate.md) | Inherited hosted generation API (`generate.*`), unavailable in default local mode |
...
5. **Resolve**: every `src` resolves in the background. Paths and asset ids load; a `<captions>` without `src` transcribes its scene. In a configured hosted build, `generate.*` declarations run in dependency order. Each unresolved element shows a pending state and carries the reason if loading fails (see [generate.md](./generate.md), [errors.md](./errors.md)). The composition stays editable while sources load.
