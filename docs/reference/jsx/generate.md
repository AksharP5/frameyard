...
The `generate.*` and `transform.*` declarations below belong to the inherited
hosted generation API. Frameyard's default local mode cannot fulfill them.
For local projects, import media generated by your own tools or ask the in-app
Assistant to generate an image through your provider account.

In a separately configured hosted build, the `generate` namespace from
`@diffusionstudio/jsx` declares assets as values. A declaration returns an
`AssetRef` that can be passed to `src`, `startFrame`, `endFrame`, or `refs`.
