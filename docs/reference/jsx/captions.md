...
Frameyard's default local mode transcribes with faster-whisper on the CPU,
without uploading the mix. Run [`npm run setup:local`](../../linux.md) first.
The temporary audio mix is removed after transcription; the transcript remains
in the project as editable caption media. See also
[`dapi media transcribe`](../tools/media/transcribe.md).
...
The audio itself is not part of the scene key. **Recutting a scene does not re-transcribe it**: to pick up changed audio, bump `seed` or use the editor's auto-captions action. A value used before replays that take from cache; a new value captures the current mix. Local mode additionally caches transcription by audio contents and model, so unchanged speech can reuse its words without loading Whisper. Hosted mode charges credits for a new transcription.
