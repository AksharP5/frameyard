...
Times are in seconds of source time. An `output` naming an existing directory
gets a fresh `dapi-transcript-*.json` inside it. In local mode, faster-whisper
caches the transcript by media content, model, and language on disk, so an app
restart does not re-transcribe unchanged audio. The command writes the JSON
output on every call. Run [`npm run setup:local`](../../../linux.md) first.
