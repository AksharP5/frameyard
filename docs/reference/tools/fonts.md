...
List fonts installed on this machine. These family names are valid `fontFamily` values on `<text>`; each family lists its variants. Linux uses Fontconfig's `fc-list`.
