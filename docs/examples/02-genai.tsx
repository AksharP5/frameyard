 ...
 *
 *   mkdir -p ~/Projects/genai
 *   cp docs/examples/02-genai.tsx ~/Projects/genai/index.tsx
 *   dapi open ~/Projects/genai
 *
 * This inherited hosted-service example cannot run in Frameyard's default
 * local mode. It requires a separately configured Diffusion Studio account.
 * `generate.*` declarations are pure values; work begins when the project mounts.
 * Refs form a dependency graph (keyframe -> matching second keyframe -> two
 * motion clips) that generates in topological order. <captions> reads the
 * finished scene audio after generation, so timing matches the voiceover.
 * Hosted generation consumes credits; results are cached in the project.
