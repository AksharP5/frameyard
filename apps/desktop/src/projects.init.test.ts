...
const { initProject, resolveProject } = await import("./projects");
...
describe("initProject", () => {
  ...
  it("uses one project directory for real paths, symlinks, and remembered aliases", async () => {
    const dir = join(root, "actual");
    const first = await initProject(null, dir);
    const alias = join(root, "linked");
    await symlink(dir, alias, "dir");
    const before = await readFile(join(dir, "package.json"), "utf8");
    for (const path of [alias, `${dir}/`, `${alias}/../linked`]) {
      expect(await initProject(null, path)).toMatchObject({ id: first.id, dir: await realpath(dir), name: "actual" });
      expect(await resolveProject(path)).toMatchObject({ id: first.id, dir: await realpath(dir) });
    }
    expect(await readFile(join(dir, "package.json"), "utf8")).toBe(before);
  });
