...
function idOf(tag: JsxTag): string | undefined {
  const initializer = attributeOf(tag, ID_ATTR)?.getInitializer();
  if (!initializer) return undefined;
  const literal = initializer.asKind(SyntaxKind.StringLiteral)
    ?? initializer.asKind(SyntaxKind.JsxExpression)?.getExpression()?.asKind(SyntaxKind.StringLiteral);
...
function setProp(tag: JsxTag, name: string, value: EditValue): void {
  ...
  // Normalized crop coordinates and frame times must survive saving exactly.
  const initializer = ["highlight", "preset"].includes(tagName(tag).toLowerCase()) && typeof value !== "string"
    ? `{${JSON.stringify(value)}}`
    : initializerText(value);
  if (attribute) attribute.setInitializer(initializer);
  else tag.addAttribute({ name, initializer });
}
...
class SourceWriter {
  ...
  private async saveFile(path: string, open: OpenFile): Promise<void> {
    ...
    if (text === open.text) return;
    const target = await realpath(absolute(this.context.dir, path));
    const mode = (await stat(target)).mode & 0o777;
    const temporary = `${target}.diffusion-save-${randomUUID()}`;
    try {
      // Stage beside the target. A full disk must not truncate the original.
      await writeFile(temporary, text, { encoding: "utf8", flag: "wx", mode });
      await chmod(temporary, mode);
      if (await realpath(absolute(this.context.dir, path)) !== target || await readFile(target, "utf8") !== open.text) {
        throw new Error("The project source changed during saving. Retry to apply the edits to the current source.");
      }
      ...
      await unlink(temporary).catch((error: NodeJS.ErrnoException) => {
        if (error.code !== "ENOENT") console.error("Could not remove staged project source", error);
      });
    }
  }
  ...
  private stampFile(path: string): void {
    ...
    // rather than once per element. An expression id is not a source locator,
    // but it is still an authored attribute and must not be duplicated.
    const changes = tags(sourceFile).flatMap((tag) => {
      if (!isCompositionTag(tagName(tag)) || attributeOf(tag, ID_ATTR)) return [];
  ...
  public async applyEdits(edits: SourceEdit[]): Promise<WriteResult> {
    ...
    for (const [file, entries] of byFile) {
      ...
      try {
        await this.saveFile(file, this.files.get(file)!);
      } catch (error) {
        skipped.push(...entries.map(editLabel));
        remaining.push(...entries);
      ...
      remaining.push(...written.remaining);
    }
  ...
  private editFile(file: string, sourceFile: SourceFile, entries: SourceEdit[]): FileWrite | undefined {
    ...
    try {
      for (const edit of entries) {
        const firstSkipped = skipped.length;
        try {
          if (edit.kind === "variable") {
          ...
          if (edit.kind === "remove") {
            if (!this.removeElement(file, sourceFile, edit, ids, removed)) skipped.push(edit.source);
            continue;
          }
          ...
          for (const [name, value] of Object.entries(edit.props)) {
            ...
            // Persist the user's edit even when the previous value was an
            // expression, so reopening and export match the live canvas.
          ...
          const declined = skipped.slice(firstSkipped);
          if (declined.length) {
            const label = editLabel(edit);
            if (edit.kind !== "set" || declined.includes(label) || declined.includes(`${label} (loop)`)) {
              remaining.push(edit);
            } else {
              // Keep only refused fields; other fields already reached disk.
              const props = Object.fromEntries(Object.entries(edit.props).filter(([name]) => declined.includes(`${label} (${name})`)));
              const { text, ...rest } = edit;
              remaining.push({ ...rest, props, ...(declined.includes(`${label} (text)`) ? { text } : {}) });
            }
          }
        }
      }
    ...
    return this.files.has(file) ? { skipped, ids, unrolled, remaining } : undefined;
  }
   ...
   * does not name an element of this file, unless its stable id was already
   * removed in this batch. Undo can remove a parent before its children.
   */
  private removeElement(
    ...
    const tag = locator === undefined ? undefined : findTag(sourceFile, locator);
    if (!tag) {
      return typeof locator === "string" && removed.has(locator)
        && !tags(sourceFile).some(candidate => idOf(candidate) === locator);
    }
    // Cutting a looped element would cut it from every iteration.
    if (inLoop(tag)) return false;

    matchIndentation(sourceFile);
    const removedIds = tagsIn(elementOf(tag)).flatMap(candidate => idOf(candidate) ?? []);
    cutElement(sourceFile, elementOf(tag));
    for (const id of removedIds) removed.add(id);
    return true;
  }
...
// Queue before reading source: concurrent checks can both pass before either rename.
async function withSourceWriter<T>(context: SourceContext, operation: (writer: SourceWriter) => Promise<T>): Promise<T> {
  const dir = await realpath(context.dir);
  const previous = sourceWrites.get(dir) ?? Promise.resolve();
  const writing = previous.catch(() => {}).then(() => operation(new SourceWriter({ ...context, dir })));
  sourceWrites.set(dir, writing);
  try { return await writing; }
  finally { if (sourceWrites.get(dir) === writing) sourceWrites.delete(dir); }
}

/** Stamps every source file of the project. Writes nothing to a fully named project. */
export function stampProject(context: SourceContext): Promise<void> {
  return withSourceWriter(context, async (writer) => writer.stampProject(await sourceFiles(context.dir)));
}
...
export function applyEdits(context: SourceContext, edits: SourceEdit[]): Promise<WriteResult> {
  return withSourceWriter(context, (writer) => writer.applyEdits(edits));
}
