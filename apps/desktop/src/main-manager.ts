...
class MainBridge {
  ...
  >();
  private bound = false;
  private trustedSender: (event: IpcMainEvent) => boolean = () => false;

  authorizeSender(check: (event: IpcMainEvent) => boolean): void {
    this.trustedSender = check;
  }
  ...
  private bind(): void {
    ...
    ipcMain.on(MAIN_WIRE.REQUEST, async (event, raw: MainRequest) => {
      if (!this.trustedSender(event)) return;
      if (!raw || typeof raw.id !== "string" || typeof raw.channel !== "string") return;
