...
describe("toml configs (codex)", () => {
  ...
  it("preserves an indented table after ours", () => {
    const before = '[mcp_servers.diffusion]\nurl = "old"\n\n  [features]\nmodel = "keep"\n';
    const after = upsertServer(before, "toml", { url: spec.url });
    expect(after).toContain('  [features]\nmodel = "keep"\n');
    expect(removeServer(before, "toml")).toContain('  [features]\nmodel = "keep"\n');
  });

  it("replaces an indented or empty table without duplicating it", () => {
    const indented = '  [mcp_servers.diffusion]\nurl = "old"\n';
    expect(readServer(indented, "toml")).toEqual({ url: "old" });
    expect(upsertServer(indented, "toml", { url: spec.url })).toBe(`[mcp_servers.diffusion]\nurl = "${spec.url}"\n`);
    expect(upsertServer('[mcp_servers.diffusion]', "toml", { url: spec.url })).toBe(`[mcp_servers.diffusion]\nurl = "${spec.url}"\n`);
  });
