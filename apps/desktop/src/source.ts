...
const AMBIGUOUS_SVG_TAGS: ReadonlySet<string> = new Set(["rect", "text", "image", "path", "ellipse"]);
        ...
        program.traverse({
          JSXElement(path) {
            ...
            opening.attributes = opening.attributes.filter((attribute) => {
              ...
              if (value?.type === "StringLiteral") locator = value.value;
              if (value?.type === "JSXExpressionContainer" && value.expression.type === "StringLiteral") locator = value.expression.value;
              return false;
            });
