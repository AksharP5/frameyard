...
const FALLBACK_INSTRUCTIONS =
  "Frameyard, a local video editor, is running on this machine and you are connected to it. The tools are the whole API; their descriptions are authoritative.";
...
export function instructions(docsDir: string | null): string {
  ...
  if (docsDir) {
    parts.push(
      `The docs, including every page named above, are the files at \`${docsDir}\`: the skills, tool and JSX references, guides, and runnable examples. They belong to the app; read them, never edit them.`,
    );
  }
  return parts.join("\n\n");
}
