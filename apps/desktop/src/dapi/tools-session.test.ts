...
beforeAll(async () => {
  ...
  serveCatalog(session, async (tool, args) => {
    ...
    switch (tool.name) {
      ...
      case "agent_tool":
        return (args as { name: string }).name === "editor_capture"
          ? { success: true, contentItems: [{ type: "inputImage", imageUrl: "data:image/png;base64,aGVsbG8=" }] }
          : { success: false, contentItems: [{ type: "inputText", text: "Element not found: title" }] };
      default:
        return {};
    }
  });
...
describe("serveCatalog", () => {
  ...
  it("presents editor captures as MCP images and preserves the structured output", async () => {
    const result = await client.callTool({ name: "agent_tool", arguments: { name: "editor_capture", args: {} } });
  ...
  it("marks editor failures as MCP errors and preserves their useful details", async () => {
    const result = await client.callTool({ name: "agent_tool", arguments: { name: "editor_update", args: { id: "title" } } });
    expect(result.isError).toBe(true);
    expect(result.content).toEqual([{ type: "text", text: "Element not found: title" }]);
    expect(result.structuredContent).toEqual({
      success: false,
