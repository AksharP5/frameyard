...
import { cp, link, mkdir, readdir, readFile, realpath, rename, rm, stat, writeFile } from "node:fs/promises";
...
async function describe(dir: string): Promise<ProjectInfo | null> {
  ...
  // The editor and agent reservations must identify symlink aliases as one project.
  const canonicalDir = await realpath(dir);
  const name = basename(canonicalDir);
...
export async function defaultRoot(window: BrowserWindow | null): Promise<string | null> {
  const dir = join(app.getPath("videos"), "Frameyard");
...
A Frameyard project. Its \`index.tsx\` file is a [Solid](https://www.solidjs.com)
composition. Frameyard compiles it into editable timeline and canvas layers.
Open only projects you trust: project code and project-local Babel plugins run
on your machine.
...
| \`package.json\` | Project identity, export settings, and dapi scripts. |
| \`tsconfig.json\` | Types for the composition tags, through \`jsxImportSource\`. |
| \`assets.yml\` | The asset library and media metadata. |
| \`assets/\` | Media stored inside the project. Linked imports can remain at their original locations. |
...
- Hosted image, video, voice, and audio generation is unavailable in the
  default local build. Use local media files or external tools you configure.
...
| \`context\` | \`dapi context\` | Open project and playhead. |
| \`check\` | \`dapi check <scene-id>\` | Check a scene before export. |
| \`capture\` | \`dapi capture <id>\` | Render frames of a scene, as an export would, to labelled PNG contact sheets. |
| \`export\` | \`dapi export <scene-id> <output>\` | Render a scene to a video file. |
...
- [JSX reference](https://github.com/AksharP5/frameyard/blob/main/docs/reference/jsx/README.md)
- [Tool and CLI reference](https://github.com/AksharP5/frameyard/blob/main/docs/reference/tools/README.md)
- [Examples](https://github.com/AksharP5/frameyard/tree/main/docs/examples)
`;
...
export async function scaffold(dir: string, displayName = basename(dir)): Promise<void> {
  ...
  if (!(await exists(join(dir, MANIFEST_FILE)))) {
    await writeManifest(dir, EMPTY_MANIFEST, { createOnly: true });
  }
}
 ...
 */
export async function writeManifest(dir: string, manifest: unknown, options: { createOnly?: boolean } = {}): Promise<void> {
  const path = join(dir, MANIFEST_FILE);
  const text = `# Frameyard asset library. Edited by the app; hand edits are read on the next load.
${stringifyYaml(manifest, { lineWidth: 0 })}`;
  ...
  try {
    ...
    await link(temp, path).catch((error: NodeJS.ErrnoException) => {
...
/** Backward-compatible project-relative spelling used by local editor paths. */
export function markSelfWrite(dir: string, path: string, content?: string | null): void {
  const absolute = join(dir, ...path.split("/"));
  if (content === undefined) known.delete(absolute);
  else noteContent(absolute, content);
}
...
export function watchProject(window: BrowserWindow | null, dir: string): void {
  ...
  const forget = (path: string): void => {
    for (const [directory, watched] of project.directories) {
      if (directory !== path && !directory.startsWith(path + sep)) continue;
      project.directories.delete(directory);
  ...
  const attach = (directory: string): void => {
    ...
    const previous = project.directories.get(directory);
    if (previous?.ino === info.ino && previous.dev === info.dev) return;
    if (previous) forget(directory);

    const watcher = watch(directory, (event, filename) => {
      if (watchers.get(dir) !== project || project.directories.get(directory)?.watcher !== watcher) return;
      const target = filename ? join(directory, filename) : directory;
      changed(target);
      if (filename && event !== "rename") return;
      try {
        if (!filename) forget(directory);
        attach(target);
      } catch (error) {
        const code = (error as NodeJS.ErrnoException).code;
        if (code === "ENOENT" || code === "ENOTDIR") forget(target);
        else unwatchProject(dir);
      }
    });
    project.directories.set(directory, { watcher, ino: info.ino, dev: info.dev });
    watcher.on("error", () => unwatchProject(dir));

    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      if (!entry.isDirectory()) continue;
      try {
        attach(join(directory, entry.name));
      } catch (error) {
        const code = (error as NodeJS.ErrnoException).code;
