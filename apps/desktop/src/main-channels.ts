...
import type { EditorToolRequest } from "./editor-agent-contracts";
import type { AssetSearchRequest, AssetSearchResult, AssetImportRequest, ImportedAgentAsset } from "./agent-asset-contracts";
import type { ManimRequest, ManimEntry } from "./manim-contracts";
import type { Checkpoint, EditorRecovery } from "./checkpoint-contracts";
import type { AudioStreamInfo, OriginalMediaRequest, OriginalVideoRequest, OriginalAudioRequest, OriginalVideoReadRequest, OriginalVideoCloseRequest, OriginalVideoInfo, OriginalVideoFrame } from "./media-original";
import type { LoudnessRequest, LoudnessResult } from "./audio-analysis";
import type { PlaybackRequest, PlaybackProgress } from "./media-playback";
import type { FindMissingMediaRequest, FindMissingMediaResult, CollectMediaRequest, CollectMediaResult } from "./media-portability";
...
export const MAIN_CHANNELS = {
  ...
  MEDIA_PREPARE_PLAYBACK: "media:prepare-playback",
  MEDIA_LIST_STREAMS: "media:list-streams",
  MEDIA_PICK_FOLDER: "media:pick-folder",
  MEDIA_FIND_MISSING: "media:find-missing",
  MEDIA_COLLECT: "media:collect",
  MEDIA_PREPARE_ORIGINAL_AUDIO: "media:prepare-original-audio",
  MEDIA_ORIGINAL_VIDEO_OPEN: "media:original-video-open",
  ...
  HYFRME_REQUEST: "hyfrme:request",
  MANIM_REQUEST: "manim:request",
  ...
  CHECKPOINTS_CREATE_RECOVERY: "checkpoints:create-recovery",
...
export type MainRequestMap = {
  ...
  [MAIN_CHANNELS.HYPERFRAMES_REQUEST]: { request: CatalogRequest; response: CatalogResponse };
  [MAIN_CHANNELS.HYFRME_REQUEST]: { request: CatalogRequest; response: CatalogResponse };
  [MAIN_CHANNELS.MANIM_REQUEST]: { request: ManimRequest; response: { items: ManimEntry[] } };
  [MAIN_CHANNELS.ANIMATION_REQUEST]: { request: AnimationRequest; response: AnimationResponse };
  [MAIN_CHANNELS.ASSETS_SEARCH]: { request: AssetSearchRequest; response: AssetSearchResult };
  [MAIN_CHANNELS.ASSETS_IMPORT]: { request: AssetImportRequest; response: ImportedAgentAsset };
  [MAIN_CHANNELS.CHECKPOINTS_LIST]: { request: { dir: string }; response: Checkpoint[] };
  [MAIN_CHANNELS.CHECKPOINTS_CREATE_RECOVERY]: { request: { dir: string; editorRecovery?: EditorRecovery }; response: Checkpoint };
  [MAIN_CHANNELS.CHECKPOINTS_CREATE]: { request: { dir: string; label: string }; response: Checkpoint };
  [MAIN_CHANNELS.CHECKPOINTS_RESTORE]: { request: { dir: string; id: string }; response: { restored: Checkpoint; recovery: Checkpoint } };
  ...
  [MAIN_CHANNELS.MEDIA_TRANSCRIBE]: {
    request: { path: string };
    response: MediaTranscribeResult;
  };
  [MAIN_CHANNELS.AUDIO_ANALYZE_LOUDNESS]: { request: LoudnessRequest; response: LoudnessResult };
  [MAIN_CHANNELS.MEDIA_LIST_STREAMS]: { request: OriginalMediaRequest; response: AudioStreamInfo[] };
  [MAIN_CHANNELS.MEDIA_PICK_FOLDER]: { request: void; response: string | null };
  [MAIN_CHANNELS.MEDIA_FIND_MISSING]: { request: FindMissingMediaRequest; response: FindMissingMediaResult };
  [MAIN_CHANNELS.MEDIA_COLLECT]: { request: CollectMediaRequest; response: CollectMediaResult };
  [MAIN_CHANNELS.MEDIA_PREPARE_ORIGINAL_AUDIO]: { request: OriginalAudioRequest; response: string };
  [MAIN_CHANNELS.MEDIA_ORIGINAL_VIDEO_OPEN]: { request: OriginalVideoRequest; response: OriginalVideoInfo };
  [MAIN_CHANNELS.MEDIA_ORIGINAL_VIDEO_READ]: { request: OriginalVideoReadRequest; response: OriginalVideoFrame };
  [MAIN_CHANNELS.MEDIA_ORIGINAL_VIDEO_CLOSE]: { request: OriginalVideoCloseRequest; response: void };
  [MAIN_CHANNELS.MEDIA_PREPARE_PLAYBACK]: { request: PlaybackRequest; response: string };
