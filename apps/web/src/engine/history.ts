 ...
 * slider, typing, repeated nudges — come as bursts of tiny transactions;
 * within this window repeats of the same value read as one adjustment.
 */
...
/**
 * The same property, text source, or variable can continue one adjustment.
 */
const slidingKey = (op: HistoryOp): string | undefined => {
	if (op.kind === 'prop') return `prop ${op.source} ${op.name}`;
	if (op.kind === 'text') return `text ${op.source}`;
...
export class EditHistory {
	...
	/** Restores the state before a gesture when its pointer is canceled. */
	...
	private replay(transaction: Transaction, ops: HistoryOp[], step: (op: HistoryOp) => void): void {
		...
		try {
			this.editor.replayEdits(() => { for (const op of ops) step(op); });
		} finally {
			this.applying = false;
			this.replaying = null;
		}
	...
	private commit(): void {
		...
		if (last && this.coalesces(last, transaction)) {
			for (const op of transaction.ops as (PropOp | TextOp | VariableOp)[]) {
				const target = last.ops.find(
					(candidate): candidate is PropOp | TextOp | VariableOp => slidingKey(candidate) === slidingKey(op),
				);
	...
	/**
	 * Whether both bursts adjust the same properties, text, or variables
	 * close enough in time. Gestures stay separate: two drags are two steps.
	 */
