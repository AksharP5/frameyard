...
export type FrameRequest = {
	...
	lastFrame: number;
	fps: number;
	/** Background thumbnails yield to preview playback. */
	canDecode?: () => boolean;
};
...
let frameDecode: Promise<void> = Promise.resolve();

function decodeFrame(sink: CanvasSink, timestamp: number, isCurrent: () => boolean) {
	const result = frameDecode.then(() => isCurrent() ? sink.getCanvas(timestamp) : null);
	frameDecode = result.then(() => {}, () => {});
	return result;
}
...
async function decodeSpread(request: FrameRequest): Promise<void> {
	...
	try {
		...
		const current = () => assetFrames.get(asset.id) === record && request.canDecode?.() !== false;

		for (let i = record.frames.length; i < count; i++) {
			try {
				const wrapped = await decodeFrame(sink, first + (i * (last - first)) / count, current);
...
async function updateClip(request: FrameRequest): Promise<void> {
	...
	try {
		...
		const sink = createSink(track, request);
		const current = () => clipFrames.get(request.clip) === cached
			&& request.canDecode?.() !== false
			&& (!cached.pending || rangeHash(cached.pending) === rangeHash(request));

		for (let i = 0; i < count; i++) {
			try {
				const wrapped = await decodeFrame(sink, ((from + i) * request.interval) / request.fps, current);
				if (!current()) return;
				if (!wrapped) continue;
		...
		cached.pending = null;
		if (pending && clipFrames.get(request.clip) === cached) void updateClip(pending);
	}
}
