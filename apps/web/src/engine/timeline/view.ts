...
/** Keep the playhead in place while zooming; bring it into view if it was offscreen. */
export function zoomTimelineView(world: World, scene: Entity, resolution: number, width: number): void {
	const frame = getCurrentFrame(world, scene);
	const position = (frame - getScrollX(world, scene)) * getResolution(world, scene);
	const anchor = position >= 0 && position <= width ? position : width / 2;
	setResolution(world, scene, resolution);
	setScrollX(world, scene, frame - anchor / resolution);
}

export function fitTimelineView(world: World, scene: Entity, width: number): void {
	if (width <= 2 * TIMELINE_PADDING_LEFT) return;
	const frames = Math.max(1, store(world, Computed).end[scene.id()] ?? 1);
	const resolution = (width - 2 * TIMELINE_PADDING_LEFT) / frames;
	setResolution(world, scene, resolution);
	setScrollX(world, scene, -TIMELINE_PADDING_LEFT / resolution);
}
