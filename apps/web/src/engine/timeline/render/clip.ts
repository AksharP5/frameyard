...
import { Audio, Caption, CaptionType, Chars, ClipDragOrigin, Computed, Hidden, Name, Selected, TrimDragOrigin, fitsChildren, getGeneratingColor, getSourceFailure, isCaption, isGenerating, isGroup, isText, store } from '@diffusionstudio/runtime';

import { getDocumentEditor } from '../../editor';
import { isClipLocked } from '../../clip-links';
...
export function renderClip(
	...
	// Sequence children share a row and can be far outside its viewport.
	// Active gestures still visit them for linked moves and marquee deselection.
	const interacting = !!surface.marquee || !!pointer.position && pointer.position.state !== 'idle';
	if (!interacting && (left + width < viewportLeft - VIEWPORT_PADDING || left > viewportRight + VIEWPORT_PADDING)) return;
...
function handleTrim(
	...
	const mode = timelineEditing(world).mode();
	if (fitsChildren(entity) || isClipLocked(entity) || mode === 'slip' || mode === 'slide') return;
	...
	if (!entity.has(TrimDragOrigin)) {
		getDocumentEditor(world).select(entity, { linked: !pointer.altPressed });
		beginTrim(world, entity);
	}

	if (entity.has(TrimDragOrigin)) applyTrim(world, surface, entity, edge, resolution);
}
...
function renderContent(
	...
	if (entity.has(Audio) && (asset?.type === 'AUDIO' || asset?.type === 'VIDEO')) {
		renderWaveform(world, scene, surface, entity, row, {
			asset, color: style.primary ?? style.foreground,
			offsetY: tall ? CLIP_LABEL_HEIGHT : 2, padding: 4,
		});
		return;
	}
...
function handleBody(
	...
	const { clicked, pressed, dragging, intersectsMarquee } = pointer.region(left, 0, width, row.height);
	const selected = entity.has(Selected);
	if (pressed && (!selected || pointer.shiftPressed || pointer.altPressed || !editor.linkedSelection)) {
		editor.select(entity, { extend: pointer.shiftPressed, linked: !pointer.altPressed });
	}
	...
	if (dragging && !entity.has(ClipDragOrigin) && !entity.has(TrimDragOrigin)) {
		...
		applyClipDrag(world, surface, resolution);
	}
	...
	if (surface.marquee) {
		if (intersectsMarquee && !selected) editor.select(entity, { extend: true, linked: false });
