...
export function renderVideoThumbnails(
	...
	const hasAudio = !!asset.channels && !entity.has(Muted);
	const layout = videoLayout(row.height, hasAudio);

	if (hasAudio) renderWaveform(world, scene, surface, entity, row, {
		asset,
		color,
	...
	requestFrames({
		canDecode: () => !scene.get(Playback)?.playing,
		clip: eid,
		asset,
		fps,
		firstFrame,
 ...
 */
function videoLayout(rowHeight: number, hasAudio: boolean) {
	const body = rowHeight - CLIP_LABEL_HEIGHT;
	const stripHeight = hasAudio ? (body > MIN_STRIP_HEIGHT ? body * 0.5 : 0) : Math.max(0, body);
