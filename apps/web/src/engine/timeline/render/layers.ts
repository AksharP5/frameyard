...
import { getResolution, getScrollY, getViewport, pixelsToFrames } from '../view';
import { RULER_HEIGHT, VIEWPORT_PADDING } from '../config';
...
export function renderLayers(world: World, scene: Entity, surface: TimelineSurfaceState): void {
	...
	const scrollY = getScrollY(world, scene);
	const minY = scrollY - VIEWPORT_PADDING;
	const maxY = scrollY + surface.layout.height - RULER_HEIGHT + VIEWPORT_PADDING;
	// Row drawing also handles gestures. Keep every row participating until
	// release, including offscreen linked clips and marquee deselection.
	const position = surface.pointer?.position;
	const interacting = !!surface.marquee || !!position && position.state !== 'idle';
	...
	const walk = (nodes: TimelineNode[], parent: Entity | null): void => {
		for (const node of nodes) {
			...
			if (node.kind === 'keyframe-track') {
				if (visible) renderKeyframeTrack(world, scene, surface, node.entity, row);
				row.top += row.height;
				continue;
			}
			...
			if (visible && (parent === null || !parent.has(Sequential))) {
