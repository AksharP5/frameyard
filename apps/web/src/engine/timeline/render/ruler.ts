...
import { Playback } from '@diffusionstudio/runtime';
...
export function renderRuler(world: World, scene: Entity, surface: TimelineSurfaceState): void {
	...
	const { dragging, clicked, pressed } = pointer
		.scope('ruler')
	...
	if (pressed) {
		...
		// Freeze the targets before the playhead follows the range. Otherwise
		// each new edge would stick to its own previous frame.
		snapFrames = getSnapFrames(world, true);
		const initial = pixelsToFrames(pointer.position.initialX + scrollX * resolution, resolution);
		rangeStart = Math.max(0, nearestSnap(snapFrames, [initial], resolution)?.frame ?? initial);
	}
	...
	if (dragging && pointer.shiftPressed) {
		...
		const wanted = Math.max(0, pixelsToFrames(pointer.position.currentX + scrollX * resolution, resolution));
		const snap = nearestSnap(snapFrames, [wanted], resolution);
		const edge = snap?.frame ?? wanted;
		const start = Math.min(rangeStart, edge);
		const end = Math.max(rangeStart, edge);
		if (snap) surface.snapX = framesToPixels(snap.frame, resolution);

		if (start < end) editWorkarea(world, scene, [start, end]);

		// The playhead follows the edge being dragged, so the frame under the
		// pointer is the one on the canvas.
		if (scene.get(Playback)?.playing) scene.set(Playback, { playing: false });
		editPlayhead(world, scene, edge);
	}
