 ...
 * with the dragged clips themselves. Workarea gestures include selected clips
 * because selecting a clip does not make it move with the range.
 */
export function getSnapFrames(world: World, includeSelected = false): number[] {
	...
	for (const entity of descendantsOf(world, scene)) {
		if (excluded.has(entity) || (!includeSelected && entity.has(Selected))) continue;
