...
import { clipRoots, editableClipTargets, expandLinkedClips, isClipLocked } from './clip-links';
...
function captureNode(world: World, entity: Entity): CapturedNode | undefined {
	...
	// Selection can be newer than the authored props while its file write is
	// pending. Undo of a deletion restores what was actually selected.
	if (entity.has(Selected)) element.props.selected = true;
	else delete element.props.selected;
...
/** Copies keep their internal links while becoming independent of the originals. */
function freshTreeLinks(tree: AuthoredTree, links: Map<string, string>): AuthoredTree {
	const props = { ...tree.props };
	const link = props.link;
	if (typeof link === 'string' && link) {
		if (!links.has(link)) links.set(link, crypto.randomUUID());
		props.link = links.get(link)!;
	}
	return { ...tree, props, children: tree.children.map((child) => freshTreeLinks(child, links)) };
}

const LOCK_CONTROLS = new Set(['locked', 'selected', 'active', 'expanded', 'clipHeight', 'muted', 'hidden', 'timeline']);
...
export class DocumentEditor {
	...
	/** Alt-selection edits the chosen clips independently until the next normal selection. */
	public linkedSelection = true;
	private readonly primary = createSignal<Entity | null>(null);
	private replaying = false;
	/** The explicitly clicked clip remains the inspector target when its partners are selected. */
	public readonly primarySelection = this.primary[0];

	public constructor(world: World) {
		this.world = world;
	}

	/** History restores authored state even when the restored state contains locks. */
	public replayEdits(action: () => void): void {
		const previous = this.replaying;
		this.replaying = true;
		try { action(); } finally { this.replaying = previous; }
	}

	private protected(entity: Entity): boolean {
		return !this.replaying && getEntityTree(this.world, entity).some(isClipLocked);
	}
	...
	public editProperty(entity: Entity, name: string, value: PropValue | AssetRef): void {
		if (!LOCK_CONTROLS.has(name) && !this.replaying
			&& (isClipLocked(entity) || (!isScene(entity) && this.protected(entity)))) return;
	...
	public editText(entity: Entity, text: string): void {
		if (this.protected(entity)) return;
	 ...
	 * The primary selection's root scene becomes the timeline's active scene.
	 */
	public select(entities: Entity | Entity[], options: { extend?: boolean; linked?: boolean } = {}): void {
		this.linkedSelection = options.linked !== false;
		const targets = Array.isArray(entities) ? entities : [entities];
		const next = new Set(this.linkedSelection ? clipRoots(expandLinkedClips(this.world, targets)) : targets);
		...
		const primary = targets.find((entity) => entity.isAlive() && entity.has(Selected))
			?? (options.extend ? this.primarySelection() : null);
		this.primary[1](primary);
		for (let ancestor = primary; ancestor; ancestor = getParentEntity(ancestor)) {
			if (isScene(ancestor) && getParentEntity(ancestor)?.has(Stage)) {
				this.activate(ancestor);
				break;
			}
		}
	}
	...
	public deselect(entities: Entity | Entity[]): void {
		const targets = Array.isArray(entities) ? entities : [entities];
		for (const entity of this.linkedSelection ? expandLinkedClips(this.world, targets) : targets) {
			this.setSelected(entity, false);
		}
		const primary = this.primarySelection();
		if (!primary?.isAlive() || !primary.has(Selected)) this.primary[1](this.world.query(Selected)[0] ?? null);
	}
	...
	public insertElement(parent: Entity, element: () => unknown, anchor?: Entity): Entity[] {
		if (!parent.get(Source)?.value || (!this.replaying && isClipLocked(parent))) return [];
	...
	public duplicate(entities: Entity | Entity[]): Entity[] {
		...
		const targets = Array.isArray(entities) ? entities : [entities];
		for (const source of this.subtreeRoots(editableClipTargets(this.world, targets))) {
			...
			copies.push(...this.insertElement(parent, () => renderAuthored(freshTreeLinks(tree, links)), anchor));
		}
	...
	public duplicateInPlace(entities: Entity | Entity[]): { original: Entity; copy: Entity }[] {
		...
		for (const original of this.subtreeRoots(entities)) {
			if (this.protected(original)) continue;
			...
			const [copy] = this.insertElement(parent, () => renderAuthored(freshTreeLinks(tree, links)), anchor);
	...
	public wrap(entities: Entity | Entity[], element: () => unknown): Entity | null {
		...
		if ([...roots].some((entity) => this.protected(entity))) return null;
		const first = [...roots][0];
	...
	public removeIntrinsicPaint(node: Entity): Entity | null {
		if (this.protected(node)) return null;
	...
	public copy(entities: Entity | Entity[]): void {
		const targets = Array.isArray(entities) ? entities : [entities];
		const roots = this.subtreeRoots(this.linkedSelection ? expandLinkedClips(this.world, targets) : targets);
	...
	public paste(parent: Entity, anchor?: Entity): Entity[] {
		if (!this.replaying && isClipLocked(parent)) return [];
	...
	public reparent(entity: Entity, parent: Entity, anchor?: Entity): boolean {
		if (!entity.get(Source)?.value || !parent.get(Source)?.value
			|| this.protected(entity) || (!this.replaying && isClipLocked(parent))) return false;
	...
	public remove(entities: Entity | Entity[]): Entity[] {
		const doomed = new Set(
			(Array.isArray(entities) ? entities : [entities]).filter(
				(entity) => entity.isAlive() && !entity.has(Stage) && !!entity.get(Source)?.value && !this.protected(entity),
			),
		);
