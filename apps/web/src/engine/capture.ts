...
import {
	Ai, ChildOf, FramePromises, FrameRate, ProjectFrameRate, Fonts, Hidden, Library, OriginalMedia, Mode, Project,
...
export interface CaptureOptions {
	...
	dir?: string;
	/** Reuse one compiled revision across successive windows of the same capture. */
	code?: string;
}
...
export interface Capture {
	/** The compiled revision used by this capture. */
	code: string;
...
export async function createCapture(source: World, node: Entity, options: CaptureOptions = {}): Promise<Capture> {
	...
	world.set(OriginalMedia, source.get(OriginalMedia) ?? null);
	...
	world.set(ProjectFrameRate, { value: source.get(ProjectFrameRate)?.value ?? source.get(FrameRate)?.value ?? 30 });
	...
	return { code, world, node: rendered, dispose: dispose(mounted) };
}
