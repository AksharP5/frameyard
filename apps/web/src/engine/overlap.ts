...
import { ChildOf, Computed, Geometry, Group, Sequential, getNextName, getParentNode, isGroup, store } from '@diffusionstudio/runtime';
...
function resolveEntityOverlap(
	...
	if (!timing || timing.end <= occStart || timing.start >= occEnd) return;
	const peers = editableClipTargets(world, [entity]).filter((peer) => peer.isAlive() && !ignore.has(peer));
	const splits = peers.filter((peer) => {
		if (isGroup(peer)) return false;
		const timing = peer.get(Computed);
		return timing && timing.start < occStart && timing.end > occEnd;
	});
	// Copy every affected partner together so their remaining tails share one
	// fresh link token, independent of the heads kept before the overwrite.
	const copies = new Map(editor.duplicateInPlace(splits).map(({ original, copy }) => [original, copy]));
	for (const peer of peers) resolvePeerOverlap(world, editor, peer, occStart, occEnd, ignore, copies.get(peer));
	for (const [original, copy] of copies) {
		const parent = getParentNode(original);
		if (parent && !isGroup(parent)) editor.wrap([original, copy], () => renderAuthored({
			tag: 'sequence', props: { name: getNextName(world, 'Sequence') }, children: [],
		}));
	}
}

function resolvePeerOverlap(
	world: World,
	editor: DocumentEditor,
	entity: Entity,
	occStart: number,
	...
	if (startCovered && endCovered) {
		...
		trimOut(world, entity, occStart);
		if (copy) trimIn(world, copy, occEnd);
	}
}
