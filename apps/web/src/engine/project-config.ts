...
import { isFrameRate, parseSource } from '@diffusionstudio/jsx';
 ...
 * publish changes after the serialized write succeeds. Fields the app does not know are carried through a write
 * untouched.
 */
...
export class ProjectConfig {
	...
	private readonly rate = createSignal(30);
	private readonly preview = createSignal<PreviewScale>(1);
	private readonly ranges = createSignal<Record<string, SourceRange>>({});
	private writing: Promise<void> = Promise.resolve();
	public readonly ready: Promise<void>;
	public readonly frameRate = () => this.rate[0]();
	public readonly previewScale = () => this.preview[0]();
	public sourceRangeOf(asset: Asset): SourceRange | undefined { return this.ranges[0]()[asset.id]; }
	public setSourceRange(asset: Asset, range: SourceRange): Promise<void> {
		if (!Number.isFinite(range.in) || !Number.isFinite(range.out) || range.in < 0 || range.out <= range.in
			|| !('duration' in asset) || range.out > asset.duration + 0.000001) return Promise.reject(new Error('The source range must be inside the clip'));
		return this.update((raw) => ({ ...raw, sourceRanges: { ...(isRecord(raw.sourceRanges) ? raw.sourceRanges : {}), [asset.id]: range } }));
	}
	public setPreviewScale(previewScale: PreviewScale): Promise<void> {
		if (![1, 0.5, 0.25].includes(previewScale)) return Promise.reject(new Error('Invalid preview scale'));
		return this.update((raw) => ({ ...raw, previewScale }));
	}
	...
	public async load(): Promise<void> {
		await this.writing;
		const value = await readProjectConfig(this.dir);
		if (this.disposed) return;
		this.apply(isRecord(value) ? value : {});
	}

	public async settle(): Promise<void> { await this.ready; await this.writing; }

	public setFrameRate(frameRate: number): Promise<void> {
		if (!isFrameRate(frameRate)) return Promise.reject(new Error('Frame rate must be between 1 and 240 fps'));
		return this.update((raw) => ({ ...raw, frameRate }));
	}

	private apply(raw: Record<string, unknown>): void {
		if (raw.frameRate !== undefined && !isFrameRate(raw.frameRate)) throw new Error('Invalid project frame rate');
		if (raw.previewScale !== undefined && raw.previewScale !== 1 && raw.previewScale !== 0.5 && raw.previewScale !== 0.25) throw new Error('Invalid preview scale');
		this.preview[1](raw.previewScale ?? 1);
		const ranges: Record<string, SourceRange> = {};
		if (isRecord(raw.sourceRanges)) for (const [id, range] of Object.entries(raw.sourceRanges)) {
			if (isRecord(range) && typeof range.in === 'number' && Number.isFinite(range.in) && range.in >= 0
				&& typeof range.out === 'number' && Number.isFinite(range.out) && range.out > range.in) ranges[id] = { in: range.in, out: range.out };
		}
		this.ranges[1](ranges);
		this.raw = raw;
		this.exports[1](parseExports(raw.export));
		this.rate[1](isFrameRate(raw.frameRate) ? raw.frameRate : 30);
	}

	private update(change: (raw: Record<string, unknown>) => Record<string, unknown>): Promise<void> {
		const writing = this.writing.then(async () => {
			const next = change(this.raw);
			await writeProjectConfig(this.dir, Object.keys(next).length ? next : null);
			if (!this.disposed) this.apply(next);
		});
	...
	/**
	 * Sets how `scene` is exported (null for none), after saving it to disk. No-op for a scene without an id (nothing mounted).
	 */
	public async setExport(scene: Entity, settings: ExportConfig | null): Promise<void> {
		...
		await this.update((raw) => {
			const exports = isRecord(raw.export) ? { ...raw.export } : {};
