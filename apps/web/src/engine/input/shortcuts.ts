...
export function deleteSelection(world: World): void {
	const selected = selectedClips(world);
...
export function pasteSelection(world: World): void {
	...
	const selected = primary?.isAlive() && primary.has(Selected) ? primary : world.query(Selected)[0];
...
export function nudgeSelection(world: World, dx: number, dy: number): void {
	...
	for (const entity of editableClipTargets(world, getSelection(world))) {
...
function toggleActivePlayback(world: World): void {
	...
	if (scene) { scene.set(Playback, { speed: 1 }); togglePlayback(world, scene); }
}
...
export function restackSelection(world: World, target: 'front' | 'back'): void {
	const editor = getDocumentEditor(world);
	const selected = new Set(selectedClips(world).filter((entity) => entity.has(Geometry) || entity.has(Group) || entity.has(AdjustmentLayer)));
...
export function selectAll(world: World): void {
	const scene = getActiveEntity(world);
	if (timelineEditing(world).focused && scene) {
		getDocumentEditor(world).select(getEntityTree(world, scene).filter((entity) => entity !== scene && !isSequence(entity)
...
const PRESSED_SHORTCUTS: readonly Shortcut[] = [
	...
	{ keys: ['x', '!mod', '!alt', '!shift'], action: deleteSelection },
	{ keys: ['x', '!mod', '!alt', 'shift'], action: rippleDeleteSelection },
	{ keys: ['q', '!mod', '!alt', '!shift'], action: (world) => trimSelectionAtPlayhead(world, 'in') },
	{ keys: ['e', '!mod', '!alt', '!shift'], action: splitAtPlayhead },
	{ keys: ['w', '!mod', '!alt', '!shift'], action: (world) => trimSelectionAtPlayhead(world, 'out') },
	{ keys: ['d', 'mod', '!shift'], action: duplicateSelection },
	{ keys: ['g', 'mod', '!shift'], action: groupSelection },
	{ keys: ['g', 'mod', 'shift'], action: ungroupSelection },
	{ keys: ['l', 'mod', '!shift', '!alt'], action: linkSelection },
	{ keys: ['l', 'mod', 'shift', '!alt'], action: unlinkSelection },
	...
	{ keys: ['j', '!mod'], action: (world) => shuttle(world, -1) },
	{ keys: ['k', '!mod'], action: (world) => shuttle(world, 0) },
	{ keys: ['l', '!mod'], action: (world) => shuttle(world, 1) },
...
const TIMELINE_SHORTCUTS: readonly Shortcut[] = [
	{ keys: ['z', '!mod', '!alt', 'shift'], action: (world) => {
		const scene = getActiveEntity(world);
		const surface = world.get(TimelineSurface);
		if (!scene || !surface?.canvas) return;
		fitTimelineView(world, scene, surface.layout.width);
	...
	{ keys: ['arrowleft', '!mod', '!alt', '!shift'], action: (world) => seekTimelineFrames(world, -1) },
	{ keys: ['arrowright', '!mod', '!alt', '!shift'], action: (world) => seekTimelineFrames(world, 1) },
	{ keys: ['arrowleft', '!mod', '!alt', 'shift'], action: (world) => seekTimelineFrames(world, -10) },
	{ keys: ['arrowright', '!mod', '!alt', 'shift'], action: (world) => seekTimelineFrames(world, 10) },
	{ keys: ['arrowup', '!mod', '!alt', '!shift'], action: (world) => seekBoundary(world, -1, 'cut') },
	{ keys: ['arrowdown', '!mod', '!alt', '!shift'], action: (world) => seekBoundary(world, 1, 'cut') },
	{ keys: ['arrowup', '!mod', '!alt', 'shift'], action: (world) => seekBoundary(world, -1, 'marker') },
	{ keys: ['arrowdown', '!mod', '!alt', 'shift'], action: (world) => seekBoundary(world, 1, 'marker') },
	{ keys: ['home', '!mod'], action: (world) => seekBoundary(world, -1, 'scene') },
	{ keys: ['end', '!mod'], action: (world) => seekBoundary(world, 1, 'scene') },
	{ keys: ['j', '!mod', '!alt'], action: (world) => shuttle(world, -1) },
	{ keys: ['k', '!mod', '!alt'], action: (world) => shuttle(world, 0) },
	{ keys: ['l', '!mod', '!alt'], action: (world) => shuttle(world, 1) },
	{ keys: ['i', '!mod', '!alt'], action: (world) => markRange(world, 'in') },
	{ keys: ['o', '!mod', '!alt'], action: (world) => markRange(world, 'out') },
	{ keys: ['o', '!mod', 'alt'], action: (world) => markRange(world, 'clear') },
	{ keys: ['m', '!mod', '!alt'], action: addMarker },
];
