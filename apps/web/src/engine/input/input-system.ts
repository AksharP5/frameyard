...
import { HitRegions, Tool, ToolType, Dragging, Hovering } from '@diffusionstudio/runtime';
...
import { pickCanvasRegion } from './picking';
...
export function inputSystem(world: World) {
	...
	for (const event of queue) {
		...
		if (event.type === 'dblclick' && world.get(Keys)?.held.has('alt')) continue;

		const region = pickCanvasRegion(world, regions, event, event.type === 'pointerdown' && (world.get(Keys)?.held.has('alt') ?? false));
