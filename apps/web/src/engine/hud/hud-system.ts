...
const SNAP_COLOR = COLORS.border.scrubber;
...
function drawHeader(world: World, ctx: Ctx2D, entity: Entity, resolution: number): void {
	...
	const foreground = failure ? ERROR_COLOR : selected ? COLORS.clip.text.foreground : 'rgba(242, 242, 242, 0.64)';
	...
	if (active && header.width > badgeWidth) {
		...
		ctx.closePath();
		ctx.fillStyle = selected ? COLORS.selection.muted : COLORS.background.muted;
		ctx.fill();
	...
	if (playback && (fitted?.width ?? 0) + badgeWidth + 60 < header.width) {
		...
		if (selected) {
			ctx.globalAlpha = 0.7;
			ctx.fillStyle = COLORS.clip.text.foreground;
		} else {
...
function drawSelectionMask(world: World, ctx: Ctx2D, mask: { width: number; height: number; mat: Mat2D }, resolution: number): void {
	...
	const maskQuad = rectToQuad(mat, width, height);
	ctx.moveTo(maskQuad[0].x, maskQuad[0].y);
	for (const point of maskQuad.slice(1)) ctx.lineTo(point.x, point.y);
	ctx.closePath();
	ctx.strokeStyle = ACCENT;
	...
	for (const [id, [x, y]] of Object.entries(rotateRegions)) {
		let rotationArea = multiply2D(mat, translate2D(x, y));
		if (mat.i !== undefined) {
			const corner = transformPoint(mat, Math.max(0, x), Math.max(0, y));
			const center = transformPoint(mat, width / 2, height / 2);
			const length = Math.hypot(corner.x - center.x, corner.y - center.y) || 1;
			rotationArea = translate2D(corner.x + (corner.x - center.x) / length * rotate / 2 - rotate / 2,
				corner.y + (corner.y - center.y) / length * rotate / 2 - rotate / 2);
		}
		regions.push({
			target: { kind: 'hud', id, quad: rectToQuad(rotationArea, rotate, rotate) },
	...
	for (const [id, [x, y, regionWidth, regionHeight]] of Object.entries(edgeRegions)) {
		let quad = rectToQuad(multiply2D(mat, translate2D(x, y)), regionWidth, regionHeight);
		if (mat.i !== undefined) {
			const indices = { l: [0, 3], t: [0, 1], r: [1, 2], b: [3, 2] } as const;
			const [a, b] = indices[id as keyof typeof indices].map(index => maskQuad[index]);
			const length = Math.hypot(b!.x - a!.x, b!.y - a!.y) || 1;
			const dx = -(b!.y - a!.y) / length * edgeOffset, dy = (b!.x - a!.x) / length * edgeOffset;
			quad = [{ x: a!.x + dx, y: a!.y + dy }, { x: b!.x + dx, y: b!.y + dy },
				{ x: b!.x - dx, y: b!.y - dy }, { x: a!.x - dx, y: a!.y - dy }];
		}
		regions.push({
			target: { kind: 'hud', id, quad },
	...
	for (const [id, [x, y]] of Object.entries(corners)) {
		const center = transformPoint(mat, x + handleOffset, y + handleOffset);
		const corner = mat.i === undefined ? multiply2D(mat, translate2D(x, y))
			: translate2D(center.x - handleOffset, center.y - handleOffset);
...
function drawDimensions(world: World, ctx: Ctx2D, mask: { width: number; height: number; mat: Mat2D }, resolution: number): void {
	...
	ctx.fillStyle = COLORS.selection.foreground;
