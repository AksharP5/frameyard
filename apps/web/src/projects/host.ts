...
import { batchProjectChanges } from './change-batch';
import { moveProjectRecovery } from './edit-recovery';
import { waitForProjectEdits } from './pending-saves';
...
export async function resolveProject(ref: string): Promise<ProjectInfo | null> {
	...
	for (const record of await findProjectRecords(ref)) {
		...
		if (!project) continue;
		const matchedAlias = project.dir !== record.dir && record.name === ref;
		if (project.id !== ref && project.name !== ref && !matchedAlias) continue;
		await normalizeProjectDirectory(record.dir, project);
...
export async function openProjectFolder(dir: string): Promise<ProjectInfo> {
	...
	await normalizeProjectDirectory(dir, project);
	await remember(project);
	return project;
}
...
export async function getProject(dir: string): Promise<ProjectInfo | null> {
	...
	const project = await mainBridge.call(MAIN_CHANNELS.PROJECTS_GET, { dir });
	if (project) await normalizeProjectDirectory(dir, project);
	return project;
}

async function normalizeProjectDirectory(from: string, project: ProjectInfo): Promise<void> {
	if (from === project.dir) return;
	await Promise.all([waitForProjectEdits(from), waitForProjectEdits(project.dir)]);
	moveProjectRecovery(from, project.dir);
	await moveProjectRecord(from, project.dir);
}
...
export function watchProject(dir: string, onChange: (paths: string[]) => void, debounceMs = 80): () => void {
	...
	const changes = batchProjectChanges(onChange, debounceMs);
