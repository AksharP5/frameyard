...
import { createMediaReader } from './media-playback';

import type { Manifest, ProjectFS } from '@diffusionstudio/assets';

/** Streams `blob` to an absolute path in chunks. */
async function writeBlob(path: string, blob: Blob, options?: { exclusive?: boolean }): Promise<void> {
...
export function createProjectFS(dir: string, previewScale: () => 1 | 0.5 | 0.25 = () => 1): ProjectFS {
	const separator = dir.includes('\\') ? '\\' : '/';
	const absolute = (source: string): string =>
		isAbsoluteSource(source) ? source : `${dir}${separator}${source.split('/').join(separator)}`;
	const file = (source: string) => new ElectronFileHandle(absolute(source)).getFile();
	...
	return {
		...
		file,
		mediaFile: createMediaReader(dir, file, previewScale),
		write: (path, blob, options) => writeBlob(absolute(path), blob, options),
