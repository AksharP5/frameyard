...
const modulesLoaded = new WeakMap<BaseAudioContext, Promise<void>>();

function ensureModuleLoaded(ctx: BaseAudioContext): Promise<void> {
  const existing = modulesLoaded.get(ctx);
  if (existing) return existing;
  const loading = ctx.audioWorklet.addModule(getVolumeMeterWorkletUrl()).catch((cause) => {
    modulesLoaded.delete(ctx);
    throw cause;
  });
  modulesLoaded.set(ctx, loading);
  return loading;
}
 ...
 * worklet is stopped, its source tap is removed, and the shared entry is dropped.
 */
...
function detach(gainNode: GainNode, listener: Listener) {
  ...
  shared.workletNode.port.postMessage('stop');
  shared.workletNode.port.close();
  try {
    gainNode.disconnect(shared.workletNode);
  } catch {
    /* The source bus may already have disconnected its outputs. */
  }
  try {
...
export function useVolumeMeter() {
  ...
  const connect = async (gainNode: GainNode) => {
    ...
    const release = await subscribe(gainNode, (next) => {
      if (token === subscriptionToken) setLevels(next);
    });
