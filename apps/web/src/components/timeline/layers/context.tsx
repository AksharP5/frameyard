...
import { Expanded, getEntityTree, isMask } from "@diffusionstudio/runtime";

import { useEditor, useTimelineIndex } from "@/engine/hooks";
import { useTimeline } from "@/context/timeline";
import { reparentTimelineClip } from "@/engine/timeline-editing";
...
export function LayerContextProvider(props: { children: JSX.Element }) {
  ...
  let pending: { entity: Entity; pointerId: number; startX: number; startY: number } | null = null;
  ...
  const begin = (event: PointerEvent, entity: Entity): void => {
    if (pending !== null || active !== null || isClipLocked(entity)) return;

    pending = { entity, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY };
  ...
  const handleCancel = (event: PointerEvent): void => {
    if (event.pointerId === pending?.pointerId) cleanup();
  };
  ...
  const updateTarget = (): void => {
    ...
    if (hit.kind === 'inside') {
      ...
      if (isClipLocked(node.entity)) { dwell = null; setTarget(null); return; }
    ...
    dwell = null;
    const next = resolveGap(hit.index, rows, pointerX, context);
    setTarget(next && !isClipLocked(next.parent) ? next : null);
  };

  const performDrop = (entity: Entity, drop: DropTarget): void => {
    if (!reparentTimelineClip(world, entity, drop.parent, drop.kind === 'line' ? drop.anchor : undefined)) return;
    if (drop.kind === 'inside' && !drop.parent.has(Expanded)) editor.editProperty(drop.parent, 'expanded', true);
  };
