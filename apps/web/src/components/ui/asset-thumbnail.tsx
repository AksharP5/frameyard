...
function MediaThumbnail(props: { asset: Asset; width: number; cache?: AssetCache }) {
  ...
  const loadThumbnail = async () => {
    ...
    try {
      ...
      if (cache) {
        blob = await cache.thumbnail(asset as LibraryAsset, width);
      } else {
        const file = await getAssetFile(asset as unknown as ImageAsset | VideoAsset);
        if (currentRequest !== request) return undefined;
        blob = await deriveThumbnail(file, asset.mimeType, width);
      }
      if (currentRequest !== request) return undefined;
