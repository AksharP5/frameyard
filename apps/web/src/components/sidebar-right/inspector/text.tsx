...
export function TextPanel(props: TextPanelProps) {
  ...
  const color = useDerived(() => entity().has(Color) ? entity().get(Computed)?.color ?? 0xffffff : 0xffffff);
  const hasFills = useDerived(() => !!entity().get(Cache)?.fills?.length);
  const [colorOpen, setColorOpen] = createSignal(false);
  let colorAnchor: HTMLDivElement | undefined;
  const changeColor = (value: number) => {
    const props = document().node(entity()).props;
    const property = props.color === undefined && props.fill !== undefined ? 'fill' : 'color';
    const hex = colorToHex(value);
    editor.editProperty(entity(), property, hex);
    syncKeyframe(world, editor, entity(), 'color', hex);
  };
  ...
  onCleanup(editor.onEdit(edit => {
    if (edit.kind !== 'prop' || edit.source !== entity().get(Source)?.value) return;
    if (edit.name === 'fontFamily') setSelectedFamily(entity().get(TextStyle)?.fontFamily ?? 'Inter');
    if (edit.name === 'fontWeight') setSelectedWeight(entity().get(TextStyle)?.fontWeight ?? '400');
  }));
  ...
  return (
    <PanelSection title="Text" ref={colorAnchor}>
      ...
      <ControlRow label="Style / size" contentClass="flex gap-2 items-center">
        <Select
      ...
      <Show when={textSelected() && !hasFills()}>
        <ControlRow label="Color">
          <ColorOpacityRow label="Text color" color={color()} onChangeColor={changeColor} onClick={() => setColorOpen(true)} />
        </ControlRow>
        <FloatingInspector open={colorOpen} anchorRef={() => colorAnchor}>
          <FloatingInspectorHeader>
            <FloatingInspectorTitle>Text color</FloatingInspectorTitle>
            <Button variant="ghost" size="icon-square" class="ml-auto" aria-label="Close text color" onClick={() => setColorOpen(false)}><Icon name="close-remove" /></Button>
          </FloatingInspectorHeader>
          <FloatingInspectorContent class="p-0"><ColorOpacityPicker color={color()} opacity={1} onColorChange={changeColor} withoutOpacity /></FloatingInspectorContent>
        </FloatingInspector>
      </Show>
