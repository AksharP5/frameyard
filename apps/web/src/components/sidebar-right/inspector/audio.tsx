...
import { AudioProcessingControls } from "./audio-processing";
...
export function AudioSettings(props: AudioSettingsProps) {
  ...
  const handleVolumeChange = (value: number | undefined) => {
    ...
    const next = Math.round(normalizeDb(value) * 10) / 10;
  ...
  return (
    <PanelSection title="Audio">
      ...
      >
        <div class="flex h-full gap-2">
          <div class="flex-1 flex flex-col gap-2 h-full">
            <ControlledTextField
              value={Math.round(normalizeDb(volume()) * 10) / 10}
      ...
      <AudioProcessingControls entity={entity()} />
    </PanelSection>
  );
}
