...
export function TransformSettings(props: TransformSettingsProps) {
  ...
  const depthSort = useDerived(() => authoredElement(entity())?.props.depthSort === "camera" ? "camera" : "layer");
  ...
  const showAddon = (addon: TransformAddon) => addons()[addon] ?? (addon === 'rotate' || addon === 'scale');
  ...
  return (
    ...
    >
      ...
      <ControlRow label="Depth">
        <MotionField node={entity()} property="z" label="Layer depth" axis="Z" />
      </ControlRow>
      <ControlRow label="3D tilt" contentClass="grid grid-cols-2 gap-2">
        <MotionField node={entity()} property="rotationX" label="Layer tilt X" axis="X" unit="°" step={0.1} />
        <MotionField node={entity()} property="rotationY" label="Layer tilt Y" axis="Y" unit="°" step={0.1} />
      </ControlRow>
      <Show when={isGroup(entity()) || isScene(entity())}>
        <ControlRow label="Depth order">
          <select aria-label="Depth order" class="h-8 w-full rounded-md border border-border-input bg-input px-2 text-xs outline-none focus-visible:ring-1 focus-visible:ring-primary" value={depthSort()} onChange={(event) => editor.editProperty(entity(), "depthSort", event.currentTarget.value === "camera" ? "camera" : "layer")}>
