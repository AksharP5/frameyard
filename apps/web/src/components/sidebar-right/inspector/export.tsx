...
import { changeExportFormat, getExportCodecs, getExportError, resolveExportSettings } from "@/engine/export-settings";
...
export function ExportPanel(props: ExportPanelProps) {
  ...
  const [exportError] = createResource(
    () => {
      const current = settings();
      const size = selectedResolution();
      if (!current || !size) return undefined;
      return { config: current, size };
    },
    ({ config, size }) => getExportError(config, size),
  );
  ...
  const write = (value: ProjectExportConfig | null) => {
    void config()?.setExport(entity(), value).catch((error: unknown) => {
      toast.error("Could not save export settings", { description: error instanceof Error ? error.message : String(error) });
    });
  };
  ...
  const applyTemplate = (id: string) => {
    ...
    if (next) write(resolveExportSettings(next, frameRate()?.value ?? 30));
  };
  ...
  return (
    ...
    >
      <Show when={settings()}>
        ...
        <Tooltip disabled={!exportError()}>
          <TooltipTrigger as="div" class="w-full">
            <Button class="w-full" onClick={runExport} disabled={exporting() || exportError.loading || !!exportError()}>
              Export
            </Button>
          </TooltipTrigger>
...
function ExportInspector(props: ExportInspectorProps) {
  ...
  const audioOnly = () => format() === "ogg";
  const codecs = createMemo(() => getExportCodecs(format()));
      ...
      <FloatingInspectorContent class="p-4">
        <div class="flex flex-col gap-2">
          <ControlRow label="Format">
            <Select
              ...
              onChange={(value) => value && write(changeExportFormat(config(), value))}
              itemComponent={(itemProps) => (
          ...
          <ControlRow label="Video">
            <Switch
              checked={!audioOnly() && config().video?.enabled != false}
              disabled={audioOnly()}
          ...
          <div
            ...
            classList={{
              "opacity-50": audioOnly() || config().video?.enabled == false,
              "pointer-events-none": audioOnly() || config().video?.enabled == false,
            }}
          >
            ...
            <ControlRow label="Frame rate">
              <Select
                value={config().video?.fps ?? props.frameRate}
              ...
              >
                <SelectTrigger>
                  <SelectValue>{config().video?.fps ?? props.frameRate} FPS</SelectValue>
                </SelectTrigger>
                <SelectPortal>
                  <SelectContent />
                </SelectPortal>
        ...
        <div class="flex flex-col gap-2">
          <ControlRow label="Audio">
            <Switch
              checked={audioOnly() || config().audio?.enabled != false}
              disabled={audioOnly()}
          ...
          <div
            class="flex flex-col gap-2 transition-opacity"
            classList={{
              "opacity-50": !audioOnly() && config().audio?.enabled == false,
              "pointer-events-none": !audioOnly() && config().audio?.enabled == false,
            }}
          >
            <ControlRow label="Codec">
              <Select
