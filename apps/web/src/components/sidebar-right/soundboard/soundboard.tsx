...
export function Soundboard() {
  ...
  const masterNode = () => masterBus()?.getOutput();
  const leftMeterNode = () => leftMeterBus()?.getOutput();
  const rightMeterNode = () => rightMeterBus()?.getOutput();
  ...
  return (
    <div class="soundboard flex items-stretch h-full w-full min-w-0 overflow-hidden justify-between px-4 pt-4 pb-1">
      <div class="flex flex-col items-center h-full gap-2">
        <div class="flex flex-1 min-h-0">
          <VolumeControl
            label="Left track volume"
            volume={leftMeterVolume()}
            disabled={!leftMeterEntity()}
