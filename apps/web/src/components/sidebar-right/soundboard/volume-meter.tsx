...
export function VolumeControl(props: VolumeControlProps) {
  ...
  return (
    ...
    >
      <input type="range" aria-label={props.label} aria-orientation="vertical" aria-valuetext={`${Math.round(props.volume)} dB`}
        min={SLIDER_MIN_DB} max={SLIDER_MAX_DB} step={1} value={Math.max(SLIDER_MIN_DB, Math.min(SLIDER_MAX_DB, props.volume))} disabled={props.disabled}
        title={`${props.label}: ${Math.round(props.volume)} dB. Double-click to reset.`}
        class="absolute -left-2 top-0 z-10 m-0 h-full w-7 cursor-ns-resize appearance-none rounded bg-transparent outline-none [writing-mode:vertical-lr] [direction:rtl] focus-visible:ring-1 focus-visible:ring-primary [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-0 [&::-webkit-slider-thumb]:w-0"
        onInput={(event) => props.onVolumeChange(Number(event.currentTarget.value))}
        onDblClick={() => props.onVolumeChange(0)} />
