...
export function PromptInput(props: PromptInputProps) {
  ...
  if (localMode) {
    return (
      <div class="absolute w-xl bottom-16 z-10 mx-auto left-1/2 -translate-x-1/2 flex items-center gap-4 rounded-xl border border-border bg-background p-4">
        <p class="flex-1 text-xs text-muted-foreground">
          Ask your agent to make an animation with Manim or HyperFrames, then import the render. Hosted generation is off.
        </p>
        <Button
          variant="secondary"
          onClick={async () => {
            ...
            await importFiles(lib, files, "");
            setPromptInputOpen(false);
          }}
        >
          Import media
        </Button>
        <Button variant="ghost" size="icon" aria-label="Close" onClick={() => setPromptInputOpen(false)}>
