...
import { AssetImagePreview } from "./asset-preview";
...
export function Assets(props: { showTabs?: boolean } = {}) {
  const { setLibraryOpen } = useLayout();
  ...
  const [previewAssetId, setPreviewAssetId] = createSignal<string>();
  const previewAsset = createMemo(() => {
    const id = previewAssetId();
  ...
  createEffect(() => {
    ...
    if (selected && !filteredAssets().some(asset => asset.id === selected) && !filteredPartials().some(partial => partial.id === selected)) {
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.target !== root) {
      if (deleting) event.stopPropagation();
      return;
    }
    const selected = selectedAssetId();
    const asset = filteredAssets().find(asset => asset.id === selected);
    ...
    if (deleting) {
      ...
      if (entry) {
        void library()?.remove([entry]).catch((cause: unknown) => {
          toast.error("Failed to delete", { description: cause instanceof Error ? cause.message : String(cause) });
        });
        return;
      }

      if (event.key === "Backspace" && currentFolder()) {
        handleGoToParent();
      }
    ...
    >
      <div class="h-12 shrink-0 flex items-center gap-2 px-4 border-y border-border">
        <div class="flex-1 min-w-0 flex items-center">
          <Show when={props.showTabs !== false}><SidebarTabs /></Show>
        </div>
            ...
            <div class="h-full flex flex-col items-center justify-center gap-4 px-6 pb-[88px] pt-4">
              <div class="flex flex-col items-center gap-1">
                ...
                <p class="text-xxs text-muted-foreground text-center">
                  {localMode
                    ? "Drag media here or import from your computer."
                    : "Drag here, import from your computer, or generate something new with AI."}
                </p>
              </div>
              ...
              <For each={filteredAssets()}>
                {(asset) => (
                  <LazyAssetItem
                    ...
                    onPreview={() => { if (asset.type === "IMAGE") setPreviewAssetId(asset.id); }}
                  />
                )}
              </For>
            </div>
      ...
      <Show when={previewAsset()} keyed>
        {asset => <AssetImagePreview asset={asset} onClose={() => setPreviewAssetId(undefined)} onRestoreFocus={() => root?.focus()} />}
      </Show>
    </div>
  );
}
