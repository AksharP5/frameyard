  ...
  return (
    <>
      <DropdownMenuGroup>
        ...
        <DropdownMenuItem onSelect={() => openMediaPortability('relink')}>Find missing media...</DropdownMenuItem>
        <DropdownMenuItem onSelect={() => openMediaPortability('collect')}>Collect project media...</DropdownMenuItem>
      </DropdownMenuGroup>
...
export function FileExportMenu() {
  const world = useWorld();
  const workarea = useTrait(useActiveScene(), Workarea);
    ...
    <>
      <DropdownMenuGroup>
        <DropdownMenuItem onSelect={handleExportScene}>
          {workarea() ? "Export marked range..." : "Export scene..."}
