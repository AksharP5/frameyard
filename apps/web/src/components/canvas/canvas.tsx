...
export function Canvas(props: { children?: JSX.Element }) {
  ...
  const handleDropEvent = async (event: DragEvent) => {
    ...
    if (presetId) {
      try {
        if (!scene) throw new Error("Drop the effect inside a scene");
        await addPreset(presetPlacement(world, presetId, { scene, point: localPt }));
      }
    ...
    if (event.dataTransfer?.getData(HIGHLIGHT_DRAG_TYPE) === "highlight") {
      try {
        if (!scene) throw new Error("Drop the highlight inside a scene");
        await addHighlight(highlightPlacement(world, { scene, point: localPt }));
  ...
  return (
    <div class="relative size-full bg-background">
      ...
      >
        <Toolbar />
        <div class="absolute top-3 right-3 z-20 max-w-[calc(100%-1.5rem)]">{props.children}</div>
        <DrawOverlay />
        <SceneInitOverlay />
        <EngineCanvas />
        <CameraController />
