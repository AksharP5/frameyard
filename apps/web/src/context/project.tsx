...
export function ProjectProvider(props: { project: ProjectInfo; children: JSX.Element }) {
	...
	const rename = async (displayName: string): Promise<void> => {
		if (renaming) throw new Error('A project rename is already in progress');
		renaming = true;
		const from = dir();
		try {
			const session = editorSession();
			if (session?.project.dir() === from) {
				...
				await flushProjectEdits(session.world);
				if (editorSession() !== session || dir() !== from) throw new Error('The project changed before it could be renamed');
			}
			setInfo(await renameProject(from, displayName));
		} finally {
			renaming = false;
		}
	};
