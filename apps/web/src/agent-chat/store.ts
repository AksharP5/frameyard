...
import { prepareChatDraft } from "@/components/agent/chat-draft";
import { store as settings } from "@/init";
import { createStoredSignal } from "@/lib/store";
import { projectRoute } from "@/hooks/use-project-route";
import type { ProjectInfo } from "@/projects";

import { attachmentFromPath, attachmentPaths, mergeAttachments, type Attachment } from "./attachments";
...
/** New chats whose optimistic item waits for the first snapshot. */
...
export function openChat(chatId: string): void {
  ...
  const unsubscribe = client.open(chatId, (message) => {
    if (message.type === "snapshot") {
      ...
      if (awaitingSnapshot.delete(chatId)) setState("sending", chatId, null);
      return;
    }
...
export async function refreshChats(projectId: string): Promise<void> {
  try {
    await whenOpen();
    const before = (state.lists[projectId] ?? []).map((chat) => ({ ...chat }));
    const list = await client.request("chats.list", { projectId });
    setState("lists", projectId, mergeChatList(before, state.lists[projectId] ?? [], list));
  } catch {
...
export async function send(options: SendOptions): Promise<string> {
  ...
  const selected = root.active()[options.projectId];
  ...
  try {
    ...
    if (!options.chatId) {
      batch(() => {
        if (!summaryOf(options.projectId, chatId)) {
          const now = Date.now();
          upsertSummary({
            id: chatId, projectId: options.projectId, ...options.model,
            title: options.text.trim().slice(0, 80) || "New chat",
            status: "running", createdAt: now, updatedAt: now,
          });
        }
        setState("sending", chatId, optimistic);
        setState("sending", key, null);
        awaitingSnapshot.add(chatId);
        if (root.active()[options.projectId] === selected) setActiveChat(options.projectId, chatId);
      });
    }
    openChat(chatId);
    flush();
...
/** The transcript's summary, or the list's copy while its snapshot is pending. */
...
export async function startChat(options: StartChatOptions): Promise<void> {
  const nativeCodex = localMode && options.model.harness === "codex" && !!window.desktop;
  if (options.skills?.length && !nativeCodex) throw new Error("Skills require native Codex chat in the desktop app.");
  ...
  if (localMode) await prepareChatDraft(options.project, options.model.harness);
  if (nativeCodex) {
    const text = [options.text, options.attachments.length ? `Attached files and folders:\n${options.attachments.join("\n")}` : ""].filter(Boolean).join("\n\n");
    try {
      await mainBridge.call(MAIN_CHANNELS.CODEX_REQUEST, { method: "new", input: { dir: options.project.dir } });
      await mainBridge.call(MAIN_CHANNELS.CODEX_REQUEST, { method: "send", input: {
        dir: options.project.dir,
        text,
        model: options.model.model,
        ...(options.skills?.length ? { skills: options.skills.map(({ name, path }) => ({ name, path })) } : {}),
        context: { projectDir: options.project.dir, source: "dashboard", attachments: options.attachments },
      } });
    } catch (error) {
      await prepareChatDraft(options.project, "codex", { text, model: options.model.model, skills: options.skills });
  ...
  try {
    ...
    setState("drafts", key, {
      text: [existing.text, options.text].filter(Boolean).join("\n\n"),
      attachments: mergeAttachments(existing.attachments, options.attachments.map(attachmentFromPath)),
    });
    toast.error("Could not start the chat", { description: (error as Error).message });
  }
}
