...
import type { HarnessId } from "@diffusionstudio/agent-chat";
...
export function RecentChats(props: RecentChatsProps) {
  const recent = () => (chatState.lists[props.projectId] ?? []).filter((chat) => !props.harness || chat.harness === props.harness).slice(0, RECENT);
          ...
          {(chat) => (
            ...
            >
              <Show when={chat.status === "running"}>
                <Icon name="spinner-loader" class="size-4 shrink-0 text-muted-foreground" />
              </Show>
