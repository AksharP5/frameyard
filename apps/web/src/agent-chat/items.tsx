    ...
    <div class="pr-0.5">
      ...
      >
        <Show when={props.item.status === "running"}>
          <Icon name="spinner-loader" class="size-6 shrink-0" />
        </Show>
...
export function ChatItem(props: { item: Item }) {
  ...
  const messageText = () => {
    const current = item();
    return current.kind === "user" || current.kind === "assistant" ? current.text : undefined;
  };
  return (
    <div class="group min-w-0">
      ...
      <Show when={messageText() !== undefined}>
        <button type="button" aria-label="Copy message" class="mt-1 text-[10px] text-muted-foreground opacity-0 group-hover:opacity-100 focus-visible:opacity-100" onClick={() => {
          const text = messageText();
          if (text === undefined) return;
          void navigator.clipboard.writeText(text).then(() => setCopied(text)).catch((error: Error) => toast.error("Could not copy", { description: error.message }));
        }}>{copied() === messageText() ? "Copied" : "Copy"}</button>
      </Show>
    </div>
  );
}
