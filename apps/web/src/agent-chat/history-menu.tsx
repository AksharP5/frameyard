...
import type { ChatSummary, HarnessId } from "@diffusionstudio/agent-chat";
...
export function HistoryMenu(props: HistoryMenuProps) {
  const chats = () => (chatState.lists[props.projectId] ?? []).filter((chat) => !props.harness || chat.harness === props.harness);
                ...
                {(chat) => (
                  <DropdownMenuItem tone="neutral" class="group/row" onSelect={() => props.onOpen(chat.id)}>
                    <Show when={chat.status === "running"}>
                      <Icon name="spinner-loader" class="size-6 -mr-1 shrink-0" />
                    </Show>
