...
export function resolveNode(world: World, id: string): Entity {
  return resolveSource(world, id, true);
}

/** Paints, strokes and keyframes have the same persistent source IDs as layers. */
export function resolveElement(world: World, id: string): Entity {
  return resolveSource(world, id, false);
}

function resolveSource(world: World, id: string, nodeOnly: boolean): Entity {
  ...
  const nodes = matches.filter(nodeOnly ? isNode : entity => authoredElement(entity) !== undefined);
  if (!nodes.length) {
    throw new DapiError(
      "wrong-kind",
      nodeOnly
        ? `"${id}" is not a node; target a scene, group, clip, or adjustment layer`
        : `"${id}" is not an editable native element`,
    );
  }
  if (nodes.length > 1) {
