...
import { getAssetFile, transcodeForTranscription } from "@diffusionstudio/runtime";
...
export const mediaTranscribe: ToolHandler<"media_transcribe"> = async ({ path }, ctx) => {
  ...
  if (localMode) {
    const result = await transcribeFile(await getAssetFile(asset));
    if (!result.segments.some((segment) => segment.words.length > 0)) {
