...
import { computeOutputSize, getExportFrameRange } from "@diffusionstudio/encoder";
...
function resolveFormat(path: string | undefined, config: ExportConfig): ContainerFormat {
  ...
  if (path !== undefined) {
    const absolute = window.desktop?.platform === "win32"
 ...
 * written. Omitted codecs are resolved first so the container check uses
 * the same defaults as the encoder.
 */
async function reconcileCodecs(settings: ExportConfig, format: ContainerFormat): Promise<ExportConfig> {
  ...
  const video = settings.video?.codec ?? "avc";
  const audio = settings.audio?.codec ?? "aac";
  const videoCodec = container.getSupportedVideoCodecs().includes(video) ? video : FALLBACK_VIDEO[format];
  const audioCodec = container.getSupportedAudioCodecs().includes(audio) ? audio : FALLBACK_AUDIO[format];
  return {
    ...settings,
    video: { ...settings.video, codec: videoCodec },
    audio: { ...settings.audio, codec: audioCodec },
  };
}
...
export const exportScene: ToolHandler<"export"> = async ({ id, path }, ctx) => {
  ...
  ctx.signal.throwIfAborted();
  if (ctx.requireSession().world !== world) throw new DapiError("no-project", "The export's project is no longer open.");
  ...
  try {
    ...
    if (result.type === "canceled") {
      throw new DapiError("canceled", "Export canceled");
    }
