...
/** Full live context used by the fork's selection-aware native chat UI. */
export async function getEditorContext(session: Accessor<EditorSession | null>) {
  ...
  const sceneId = active?.get(Source)?.value;
  const workarea = active?.get(Workarea);
  const library = world.get(Library);
  const selectedAssetId = world.get(AssetSelection)?.id;
  const selectedAsset = selectedAssetId ? library?.get(selectedAssetId) : undefined;

  return {
    rootDir,
    projectDir: project.dir(),
    activeScene: active ? describeNode(active) : null,
    selected: world.query(Selected).filter((node) => node.has(Source)).map(describeNode),
    selectedAsset: selectedAsset ? { path: selectedAsset.path, type: selectedAsset.type } : null,
    assets: (library?.list() ?? []).slice(0, 100).map(({ path, type }) => ({ path, type })),
    assetCount: library?.list().length ?? 0,
    ...
    sceneTiming: active && sceneId ? {
      sceneId,
      sceneName: active.get(Name)?.value || sceneId,
      frameRate,
      duration: (active.get(Computed)?.duration ?? 0) / frameRate,
      currentTime: (active.get(Computed)?.localTime ?? 0) / frameRate,
      timelineRange: workarea && workarea.end > workarea.start
        ? { start: workarea.start / frameRate, end: workarea.end / frameRate }
        : null,
    } : null,
...
function describeNode(entity: Entity) {
  const authored = authoredElement(entity);
  return {
    source: entity.get(Source)?.value ?? null,
    name: entity.get(Name)?.value ?? null,
    tag: authored?.tag ?? null,
    props: Object.fromEntries(Object.entries(authored?.props ?? {}).filter(([, value]) => isPropValue(value))),
    text: entity.get(Chars)?.value ?? null,
  };
}
