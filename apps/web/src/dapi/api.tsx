...
import { hostedServiceUnavailable, localMode } from "@/lib/local-mode";
import { toolBridge } from "./bridge";
import { handlers } from "./handlers";
import { registerAgentTools } from "./agent";
import { editorSession, requireEditorSession, setEditorSession, waitForEditorSession } from "./session";
  ...
  const context: ContextFactory = (signal) => ({
    ...
    app: {
      async openProject(dir) {
        ...
        await waitForEditorSession(project.dir, signal);
        return { id: project.id, name: project.displayName, dir: project.dir };
      },
      user: () => auth.user() ?? null,
      requireUser() {
        if (localMode) throw new DapiError("unsupported", hostedServiceUnavailable);
        const user = auth.user();
        if (!user) throw new DapiError("sign-in-required", "Sign in required: hosted AI generation needs an account.");
        return user;
      },
    },
  });
