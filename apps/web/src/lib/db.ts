...
type StoredChatDraft = Partial<Omit<ChatDraft, 'tab'>> & {
  tab?: ChatDraft['tab'] | 'Manim';
  manimReferences?: ChatDraft['animationReferences'];
};
...
export interface GlobalDBSchema extends idb.DBSchema {
  ...
  chatDrafts: {
    value: StoredChatDraft;
    key: [string, keyof StoredChatDraft];
  };
  bundles: {
    value: ProjectBundle;
    key: string;
...
// The fork already shipped a different version 3 (chat drafts + roots), so
// the upstream roots-to-projects migration needs its own version here.
const DB_VERSION = 4;

const dbPromise = openDB<GlobalDBSchema>(DB_NAME, DB_VERSION, {
  async upgrade(db, _oldVersion, _newVersion, tx) {
    if (!db.objectStoreNames.contains('chatDrafts')) {
      db.createObjectStore('chatDrafts');
    }
...
/** Migrate directory-keyed drafts once; a stable project ID survives renames. */
export async function loadChatDraft(key: string, dir: string): Promise<Partial<ChatDraft>> {
  const db = await dbPromise;
  const tx = db.transaction('chatDrafts', 'readwrite', { durability: 'strict' });
  const range = (prefix: string) => IDBKeyRange.bound([prefix, ''], [prefix, '\uffff']);
  const restore = async () => {
    const legacyKeys = await tx.store.getAllKeys(range(dir));
    const legacy = await Promise.all(legacyKeys.map((key) => tx.store.get(key)));
    const current: StoredChatDraft = Object.assign({}, ...await tx.store.getAll(range(key)));
    const { manimReferences, tab, ...fields }: StoredChatDraft = Object.assign({}, ...legacy, current);
    const animations = fields.animationReferences ?? manimReferences;
    const saved: Partial<ChatDraft> = {
      ...fields,
      ...(tab ? { tab: tab === 'Manim' ? 'Animations' : tab } : {}),
      ...(animations ? { animationReferences: animations } : {}),
    };
    const video = saved.videoContext;
    const area = saved.area;
    const previousDir = saved.projectDir ?? dir;
    const moved = (path: string, from = previousDir) => path.startsWith(`${from}/`) ? dir + path.slice(from.length) : path;
    const transcript = (item: NonNullable<ChatDraft['transcript']>) => ({ ...item, projectDir: dir, path: moved(item.path, item.projectDir) });
    const draft: Partial<ChatDraft> = previousDir === dir ? { ...saved, projectDir: dir } : {
      ...saved,
      projectDir: dir,
      ...(saved.skills ? { skills: saved.skills.map((skill) => ({ ...skill, path: moved(skill.path) })) } : {}),
      ...(saved.animationReferences ? { animationReferences: saved.animationReferences.map((item) => ({ ...item, source: moved(item.source), output: moved(item.output), ...(item.entry ? { entry: moved(item.entry) } : {}) })) } : {}),
      ...(saved.transcript ? { transcript: transcript(saved.transcript) } : {}),
      ...(video ? { videoContext: {
        ...video, projectDir: dir, path: moved(video.path, video.projectDir),
        frames: video.frames.map((frame) => ({ ...frame, path: moved(frame.path, video.projectDir) })),
        ...(video.fullTranscript ? { fullTranscript: transcript(video.fullTranscript) } : {}),
      } } : {}),
      ...(area?.snapshot.context.projectDir ? { area: { ...area, snapshot: { ...area.snapshot, context: { ...area.snapshot.context, projectDir: dir } } } } : {}),
    };
    for (const field of Object.keys(draft) as (keyof ChatDraft)[]) {
      if (draft[field] !== current[field]) await tx.store.put({ [field]: draft[field] }, [key, field]);
    }
    if (manimReferences !== undefined) await tx.store.delete([key, 'manimReferences']);
    for (const legacyKey of legacyKeys) await tx.store.delete(legacyKey);
    return draft;
  };
  const [draft] = await Promise.all([restore(), tx.done]);
  return draft;
}

export async function saveChatDraftField<Key extends keyof ChatDraft>(dir: string, field: Key, value: ChatDraft[Key]): Promise<void> {
  const db = await dbPromise;
  const tx = db.transaction('chatDrafts', 'readwrite', { durability: 'strict' });
  if (value === undefined) await tx.store.delete([dir, field]);
  else await tx.store.put({ [field]: value }, [dir, field]);
  await tx.done;
}
