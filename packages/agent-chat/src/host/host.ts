...
export type AgentHostOptions = {
  ...
  mcp: McpConfig | null;
  instructions?: string;
  /** The desktop reserves and checkpoints the project before agent edits. */
...
export class AgentHost {
  ...
  private async send(connection: Connection, params: MethodParams<"turn.send">): Promise<{ chatId: string }> {
    if (this.stopping) throw new HostError("busy", "The agent host is stopping");
    ...
    try {
      ...
      if (chat.idleTimer) clearTimeout(chat.idleTimer);
      chat.idleTimer = null;
      if (chat.session && chat.meta.cwd !== cwd) {
        await chat.session.close();
        chat.session = null;
      }
      if (this.stopping || chat.turn.interrupted) throw new HostError("busy", "Turn interrupted before starting");
  ...
  private async runTurn(chat: Chat, turnId: string, text: string, model: string, emit: Emit): Promise<void> {
    ...
    try {
      try {
        release = await this.options.prepareTurn?.({ cwd: chat.meta.cwd, text });
        if (this.stopping || chat.turn?.interrupted) throw new Error("Turn interrupted before starting");
      ...
      await this.options.store.writeMeta(chat.meta).catch((error: Error) => {
        emit({ type: "item.completed", item: { id: `n-${turnId.slice(0, 12)}-save`, kind: "notice", level: "error", text: `Could not save chat: ${error.message}` } });
        this.log(`save ${chat.meta.id} failed: ${error.message}`);
      });
      chat.turn = null;
      emit({ type: "chat.updated", chat: this.summary(chat) });
