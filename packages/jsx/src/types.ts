...
export const ANIMATABLE_PROPERTIES = {
  ...SPATIAL_PROPERTY_PATHS,
  ...SPATIAL_ARRAY_PATHS,
  x: "position.x", y: "position.y", z: "position.z",
  offsetX: "offset.x", offsetY: "offset.y",
  width: "width", height: "height",
  rotation: "rotation", rotationX: "rotation.x", rotationY: "rotation.y",
  scale: "scale", scaleX: "scale.x", scaleY: "scale.y",
  anchorX: "anchor.x", anchorY: "anchor.y", skewX: "skew.x", skewY: "skew.y",
  opacity: "opacity",
  ...
  volume: "volume", color: "color", offset: "stop.offset", blur: "blur", value: "effect.value",
  d: "path.d",
  backdropBlur: "backdropBlur", refraction: "refraction",
  bloom: "bloom", vignette: "vignette", grain: "grain", colorSplit: "colorSplit", focusDistance: "focusDistance", aperture: "aperture",
  perspective: "perspective", cameraX: "cameraX", cameraY: "cameraY", cameraZ: "cameraZ",
  cameraZoom: "cameraZoom", cameraRotationX: "cameraRotationX", cameraRotationY: "cameraRotationY", cameraRotation: "cameraRotation",
} as const;
...
type TransformProps = PositionProps & OffsetProps & SizeProps & {
  /** Blur the composited content behind this layer, px. Default 0. */
  backdropBlur?: number;
  /** Glass displacement strength, 0–1. Default 0. */
  refraction?: number;
  /** Depth relative to the parent, px. Positive moves toward the scene camera. */
  z?: number;
  scaleZ?: number;
  renderOrder?: number;
  depthTest?: boolean;
  rigidBody?: RigidBodyOptions | boolean;
  /** Rotation around the horizontal/vertical axes in degrees. */
  rotationX?: number;
  rotationY?: number;
  /** Transform pivot as a fraction of the box, 0 = start, 0.5 = center (default). */
  anchorX?: number;
  anchorY?: number;
  /** Per-axis skew in degrees. */
  skewX?: number;
  skewY?: number;
  /** Reflect across the respective axis. Defaults to false. */
  flipX?: boolean;
  flipY?: boolean;
  /**
...
type TimingProps = {
  /** Protect this clip or track from timeline edits. */
  locked?: boolean;
  /** Clips with the same link token move and edit together. */
  link?: string;
...
type FrameRateProps = {
   ...
   * project and export frame rates.
   */
  frameRate?: number;
};
...
export type ColorGradeSettings = {
  temperature?: number;
  tint?: number;
  curves?: Partial<Record<'master' | 'red' | 'green' | 'blue', [number, number][]>>;
};
...
type AudioTrackProps = {
  /** Zero-based audio stream in the source file. Defaults to the first stream. */
  audioStream?: number;
...
type CommonProps = { colorGrade?: ColorGradeSettings } & IdentityProps & TransformProps & CompositeProps & TimingProps & SequenceItemProps & Pick<AudioTrackProps, 'audioProcessing' | 'audioStream'>;
 ...
 */
export type DepthSortMode = "layer" | "camera";

export type SceneCameraProps = {
  /** Child planes use layer depth by default; camera ordering is for solid face assemblies. */
  depthSort?: DepthSortMode;
  /** Scene finishing effects. Bloom, vignette and grain use strengths 0–1. */
  bloom?: number;
  vignette?: number;
  grain?: number;
  /** Chromatic separation in pixels. Default 0. */
  colorSplit?: number;
  /** Distance from the camera to the focus plane in pixels. Default 1000. */
  focusDistance?: number;
  /** Depth-of-field blur strength in pixels. Default 0. */
  aperture?: number;
  /** Focal length in pixels. Default 1000. */
  perspective?: number;
  /** Camera position in scene coordinates. X/Y default to the scene center, Z to 1000. */
  cameraX?: number;
  cameraY?: number;
  cameraZ?: number;
  /** Camera magnification. Default 1. */
  cameraZoom?: number;
  cameraOffsetX?: number;
  cameraOffsetY?: number;
  /** Camera rotations in degrees. Default 0. */
  cameraRotationX?: number;
  cameraRotationY?: number;
  cameraRotation?: number;
};

export type SceneProps = SceneCameraProps & IdentityProps & PositionProps & Required<Pick<SizeProps, "width" | "height">> & Pick<SizeProps, "keepAspectRatio"> & FillProps & {
  /**
  ...
  markers?: { id: string; time: Time; name: string }[];
...
export type GroupProps = CommonProps & FillProps & {
  physics?: PhysicsWorldOptions | boolean;
  /** Order children by layer depth (default) or distance from the scene camera. */
  depthSort?: DepthSortMode;
  /** Element children, plus `<Effect>` (filtering the group as a whole), `<Animation>` and `<KeyframeTrack>` children. */
  children?: SolidJSX.Element;
};

/** Enlarge a live region of the composited layers below this clip. */
export type HighlightProps = IdentityProps & TimingProps & PositionProps & SizeProps & Pick<CompositeProps, "hidden"> & Partial<HighlightOptions>;

/** An editable motion graphic or live scene effect from the Effects library. */
export type PresetProps = IdentityProps & TimingProps & PositionProps & SizeProps & Pick<CompositeProps, "hidden"> & {
  preset: string;
...
/** An ellipse inscribed in its editable box. Accepts the same paints and masks as a rect. */
export type EllipseProps = RectProps;

/** SVG vector geometry in local coordinates; individual paths remain native editable layers. */
export type PathProps = RectProps & {
  d: string;
  /** How overlapping subpaths form holes. Default "nonzero", matching SVG. */
  fillRule?: "nonzero" | "evenodd";
  /** Map this SVG coordinate rectangle into width × height. Omit to use local pixel coordinates. */
...
export type GradientPaintProps = PaintProps & PositionProps & Pick<TransformProps, "scaleX" | "scaleY"> & {
  /** Explicit normalized coordinates, overriding the rotation/position handles. */
  x1?: number; y1?: number; x2?: number; y2?: number;
  centerX?: number; centerY?: number; radiusX?: number; radiusY?: number;
  focalX?: number; focalY?: number;
  gradientSpace?: "local" | "screen";
...
/** A depth-composited 3D viewport containing ordinary native layers, meshes and lights. */
export type Scene3DProps = GroupProps & SceneCameraProps & {
  width: number;
  height: number;
...
export type Path3DProps = CommonProps & FillProps & {
  /** Absolute M/L/C/Q/Z curves with x,y,z per point. Animate through property="d". */
  d: string;
  fillRule?: "nonzero" | "evenodd";
  lit?: boolean;
  children?: SolidJSX.Element;
...
export type LightProps = CommonProps & Partial<ColorProps> & {
  type?: LightType;
  intensity?: number;
  distance?: number;
  decay?: number;
...
/** Ray-marched density inside an editable 3D box. Time is the layer's local playhead. */
export type VolumeProps = CommonProps & Partial<ColorProps> & {
  depth?: number;
  density?: number;
  noiseScale?: number;
  flowSpeed?: number;
