...
export interface ProjectFS {
	...
	/** A local video's compatible playback copy, or its original bytes when supported. */
	mediaFile?(source: string): Promise<File>;
	/** Writes inside the project, creating parents. Exclusive writes must refuse an existing file. */
	write(path: string, data: Blob, options?: { exclusive?: boolean }): Promise<void>;
