...
/** The id of a sequence: frame metadata, including mtime so same-size re-renders invalidate decoders. */
export async function hashSequence(frames: { name: string; size: number; mtime?: number }[], frameRate?: number): Promise<string> {
	const text = frames.map((frame) => `${frame.name}:${frame.size}:${frame.mtime ?? 0}`).join('\n');
	const bytes = new TextEncoder().encode(`seq\n${text}${frameRate === undefined ? '' : `\nfps:${frameRate}`}`);
	return hexOf(await crypto.subtle.digest('SHA-256', bytes)).slice(0, ID_LENGTH);
}
