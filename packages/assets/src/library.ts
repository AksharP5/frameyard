...
import { parseSequenceMetadata, SEQUENCE_METADATA_FILE } from './sequence';
...
export class AssetLibrary {
	...
	/** Manifest persistence state. Failed changes remain in memory; `settle()` retries them. */
	public readonly saveState: Accessor<LibrarySaveState>;
	...
	private readonly setSaveState: (state: LibrarySaveState) => void;
	...
	public constructor(fs: ProjectFS, options: LibraryOptions = {}) {
		...
		[this.saveState, this.setSaveState] = createSignal<LibrarySaveState>({ status: 'saved' });
	}
	...
	public async load(): Promise<void> {
		// A watcher refresh must not replace unsaved relinks with the old manifest.
		await this.settle();
		const revision = this.revision;
		const manifest = normalizeManifest(await this.fs.readManifest());

		const next = new Map<string, AssetEntry>();
		const relinked = new Map<string, Asset>();
		const previous = new Map(this.listNow().map((asset) => [asset.path, asset]));
		let refreshed = false;
		await Promise.all(manifest.assets.map(async (record) => {
			...
			if (asset.id !== record.id) relinked.set(record.id, asset);
			const live = previous.get(asset.path);
			if (live && (live.id !== asset.id || live.source !== asset.source)) relinked.set(live.id, asset);
			if (asset.id !== record.id || asset.stat?.mtime !== record.stat?.mtime || asset.stat?.size !== record.stat?.size) refreshed = true;
		}));
		// Imports and edits can finish while media is being refreshed. Read their saved state again.
		...
		this.publish();
		for (const [from, asset] of relinked) this.onRelink?.(asset, from);
	...
	private async revive(record: AssetRecord): Promise<Asset> {
		...
		if (record.type === 'SEQUENCE') {
			...
			const info = await this.sequenceInfo(record.source, frames);
			if (info.id === record.id) return this.attach(record);
			return this.describeSequence(record.source, frames, { path: record.path, createdAt: record.createdAt }, info);
		}
	 ...
	 */
	public import(sources: readonly string[], options: ImportOptions = {}): Promise<ImportResult> {
		return this.importSources(sources, options, new Set());
	}

	private async importSources(sources: readonly string[], options: ImportOptions, ancestors: ReadonlySet<string>): Promise<ImportResult> {
		const result: ImportResult = { assets: [], failed: [] };
		await Promise.all(sources.map(async (source) => {
			try {
				result.assets.push(...await this.importOne(source, options, result.failed, ancestors));
			} catch (error) {
	...
	private async importOne(source: string, options: ImportOptions, failed: ImportResult['failed'], ancestors: ReadonlySet<string>): Promise<Asset[]> {
		...
		const real = await this.fs.realPath?.(source);
		if (real && ancestors.has(real)) throw new Error(`Folder contains a symbolic link cycle: ${source}`);
		const entered = real ? new Set([...ancestors, real]) : ancestors;
		...
		const imported = await this.importSources(children, { folder }, entered);
		failed.push(...imported.failed);
	...
	public async store(blob: Blob, options: ImportOptions & { name: string }): Promise<Asset> {
		...
		const partial = key === undefined ? undefined : this.partialFor(key);
		const reservedPaths: string[] = [];
		let written: string | undefined;
		try {
			const base = joinPath(options.folder ?? '', options.name);
			let path: string;
			do {
				path = this.reservePath(base, partial);
				reservedPaths.push(path);
			} while (await this.fs.stat(joinPath(ASSETS_DIR, path)));
			const source = joinPath(ASSETS_DIR, path);
			const file = new File([blob], basename(path), { type: blob.type });
			const asset = await this.describeFile(source, { path, generation: options.generation }, file);
			if (this.disposed) throw new Error('The asset library is closed.');
			const existing = this.map.get(asset.id);
			if (existing && !isPartialAsset(existing) && !existing.transient) {
				if (key !== undefined) this.dropPartial(key);
				if (options.generation && existing.generation?.key !== key) {
					this.update(existing, { generation: options.generation });
				}
				return existing;
			}
			await this.fs.write(source, blob, { exclusive: true });
			written = source;
			asset.stat = await this.fs.stat(source) ?? asset.stat;
			if (key !== undefined) this.dropPartial(key);
			const added = this.add(asset);
			if (options.generation && added.generation?.key !== key) {
				this.update(added, { generation: options.generation });
			}
			written = undefined;
			if (added.source !== source) await this.fs.remove(source);
			return added;
		} catch (error) {
			if (written) {
				try { await this.fs.remove(written); }
				catch (cleanup) { throw new AggregateError([error, cleanup], `Could not clean up unfinished asset: ${written}`); }
			}
			throw error;
		} finally { for (const path of reservedPaths) this.reservedPaths.delete(path); }
	}
	...
	private add(asset: Asset): Asset {
		...
		if (this.entriesNow().some(other => other.path === asset.path)) asset.path = this.uniquePath(asset.path);
		this.reorder(asset);
		this.changed();
		return asset;
	}
	 ...
	 * is the host's to move over (`onRelink`). Refuses bytes already registered
	 * under another path so neither asset's source references are lost.
	 */
	public async relink(asset: Asset, source: string): Promise<Asset> {
		...
		const live = this.map.get(from);
		if (!live || isPartialAsset(live) || live.source !== previousSource) {
			throw new Error('The asset was removed or changed while relinking. Try again.');
		}
		const existing = this.map.get(next.id);
		if (existing && existing !== live && (isPartialAsset(existing) || !existing.transient)) {
			throw new Error(`This file is already registered as ${existing.path}.`);
		}
		next.path = live.path;
		if (next.id === from) {
			Object.assign(live, next);
			this.changed();
			this.onRelink?.(live, from);
			return live;
		}
	...
	private changed(): void {
		...
		this.revision++;
		this.dirty = true;
		const status = this.saveState().status;
		if (status !== 'failed' && status !== 'saving') this.setSaveState({ status: 'dirty' });
		this.publish();
	...
	/** Saves in the background; failures are logged and remain available to retry. */
	public flush(): Promise<void> {
		return this.writeManifest().catch((error: unknown) => console.error('[assets] could not write the manifest:', error));
	}

	/** Persist all pending changes, retrying a failed manifest write. Rejects if saving fails again. */
	public async settle(): Promise<void> {
		while (true) {
			const saving = this.writeManifest();
			await saving;
			if (!this.dirty && saving === this.saving) return;
		}
	}
	...
	private writeManifest(): Promise<void> {
		...
		const saving = this.saving
			.catch(() => {})
			.then(() => {
				// Include changes made while this write was waiting in the queue.
				this.dirty = false;
				return this.fs.writeManifest(this.manifest());
			})
			.then(() => {
				if (saving === this.saving) this.setSaveState({ status: this.dirty ? 'dirty' : 'saved' });
			})
			.catch((error: unknown) => {
				this.dirty = true;
				if (saving === this.saving) this.setSaveState({ status: 'failed', error: error instanceof Error ? error.message : String(error) });
				throw error;
			});
		this.saving = saving;
		this.setSaveState({ status: 'saving' });
		return saving;
	}
	...
	/** Holds a name while its import is being read or written. */
	private reservePath(path: string, except?: AssetEntry): string {
		const reserved = this.uniquePath(path, except);
	...
	private uniquePath(path: string, except?: AssetEntry): string {
		const taken = new Set(this.reservedPaths);
		for (const entry of this.entriesNow()) {
			if (entry === except) continue;
			taken.add(entry.path);
			// Library renames leave stored files in place; their names stay occupied.
			if (!isPartialAsset(entry) && entry.source.startsWith(`${ASSETS_DIR}/`)) {
				taken.add(entry.source.slice(ASSETS_DIR.length + 1));
			}
		}
	...
	private async describeFile(source: string, meta: DescribeMeta, contents?: File): Promise<Asset> {
		const file = contents ?? await this.fileHandle(source).getFile();
		...
		return {
			...
			handle: this.fileHandle(source, probe.type === 'VIDEO'),
			...probe,
		};
	}
	...
	private async sequenceInfo(source: string, frames: FsEntry[]): Promise<{ id: string; frameRate: number }> {
		const path = joinPath(source, SEQUENCE_METADATA_FILE);
		const metadata = await this.fs.stat(path)
			? parseSequenceMetadata(JSON.parse(await (await this.fs.file(path)).text()))
			: undefined;
		return { id: await hashSequence(frames, metadata?.frameRate), frameRate: metadata?.frameRate ?? DEFAULT_SEQUENCE_FPS };
	}

	private async describeSequence(source: string, frames: FsEntry[], meta: DescribeMeta, info?: { id: string; frameRate: number }): Promise<SequenceAsset> {
		...
		const { id, frameRate } = info ?? await this.sequenceInfo(source, frames);
	...
	private attach(record: AssetRecord): Asset {
		...
		return {
			...record,
			handle: isUrlSource(record.source) ? this.urlHandle(record.source) : this.fileHandle(record.source, record.type === 'VIDEO'),
		};
	}

	private fileHandle(source: string, video = false): AssetFileHandle {
		return {
			getFile: () => this.fs.file(source),
			...(video && this.fs.mediaFile ? { getMediaFile: () => this.fs.mediaFile!(source) } : {}),
		};
	}
