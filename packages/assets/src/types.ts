...
export interface AssetFileHandle {
	...
	/** Optional compatible copy for decoding; getFile always returns the original. */
	getMediaFile?(): Promise<File>;
}

export function readMediaFile(asset: Pick<Asset, 'handle'>): Promise<File> {
	return asset.handle.getMediaFile?.() ?? asset.handle.getFile();
}
