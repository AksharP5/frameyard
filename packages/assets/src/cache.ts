...
import { readMediaFile } from './types';
...
export class AssetCache {
	...
	public thumbnail(asset: Asset, width = DEFAULT_THUMBNAIL_WIDTH): Promise<Blob | null> {
		...
		return this.get(THUMBNAIL, asset, async () => deriveThumbnail(await readMediaFile(asset), asset.mimeType, width), variant);
	}
	...
	public waveform(asset: Asset): Promise<File | null> {
		return this.ensure(WAVEFORM, asset, async () => deriveWaveform(await readMediaFile(asset)));
	}
