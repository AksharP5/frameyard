...
// Bound full-file audio decoding when multiple assets need waveforms at once.
const MAX_WORKERS = 2;
let activeWorkers = 0;
 ...
 */
export async function deriveWaveform(file: Blob, peaksPerSecond = WAVEFORM_PEAKS_PER_SECOND): Promise<Uint8ClampedArray | null> {
	if (activeWorkers >= MAX_WORKERS) await new Promise<void>((resolve) => waiting.push(resolve));
	else activeWorkers++;

	try {
		const worker = new Worker(new URL('./waveform-worker.ts', import.meta.url), { type: 'module' });
		return await new Promise<Uint8ClampedArray | null>((resolve, reject) => {
