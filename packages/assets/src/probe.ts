...
export async function detectMimeType(input: Blob | string): Promise<string | null> {
	...
	try {
		const source = typeof input === 'string' ? new UrlSource(input) : new BlobSource(input, { useStreamReader: false });
		const media = new Input({ formats: ALL_FORMATS, source });
		try { mimeType = await media.getMimeType(); }
...
export function sortFrames<T extends { name: string }>(frames: T[]): T[] {
	...
	return frames.filter((frame) => !frame.name.startsWith('.')).sort((a, b) => collator.compare(a.name, b.name));
}
