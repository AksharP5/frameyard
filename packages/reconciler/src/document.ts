...
import { DepthSort, LayerMaterial, SceneEffects, Anchor, Skew, Flip, SceneCamera, PathData, parsePathData, parsePathViewBox, Active, AdjustmentLayer, Animation, AnimationPhase, AnimationType, appendChild, AssetId, Audio, AudioStream, AudioProcessing, parseAudioProcessing, Background, bindAsset, BlendMode, BlendModeType, Blur, Caption, CaptionAlign, CAPTION_PRESET_FILLS, CAPTION_PRESET_STYLES, CaptionType, Chars, ClipHeight, ClipLink, Locked, Markers, ClipsContent, Computed, Constraint, ConstraintCache, ConstraintType, CornerRadius, ColorGrade, parseColorGrade, createEntity, DEFAULT_BACKGROUND, Color, ColorStop, Delay, Effect, EffectType, Expanded, FontStyle, FramePromises, FrameRate, ProjectFrameRate, Generating, GenerationRequest, getActiveEntity, Loop, LoadRequest, Geometry, GeometryType, isAncestorOf, getParentEntity, getParentNode, Hidden, Highlight, Preset, Host, IsMask, isText, ItemIndex, KeepAspectRatio, Keyframe, KeyframeTrack, MixedCornerRadius, Mode, Muted, Name, Offset, Opacity, Paint, PaintType, parseColor, PendingSource, PendingSync, Playback, PlaybackRate, Position, removeChild, RenderSurface, resizeEntity, Scale, ScaleMode, ScaleModeType, secondsToFrames, getAsset, getEntityChildren, Group, Sequential, Shader, Size, Stage, Root, Rotation, Scene, Selected, Shadow, Source, SourceFrameRate, setCameraMatrix, setPlayhead, setTimelineView, Stroke, StrokeCap, StrokeJoin, StrokeStyle, SyncRequest, TextAlign, TextBaseline, TextCase, TextRange, TextStyle, TranscriptionRequest, Transition, TransitionType, Trim, UniformScale, Volume, Workarea } from '@diffusionstudio/runtime';
import { ANIMATABLE_PROPERTIES, HIGHLIGHT_DEFAULTS, LOOP_ATTR, parseHighlightOptions, parsePresetOptions, parseTime, SOURCE_ATTR } from '@diffusionstudio/jsx';
...
import type { AuthoredElement, AuthoredTree, AnimatableProperty, AssetRef } from '@diffusionstudio/jsx';

import type { Entity, World } from 'koota';
import type { ProjectDocument, ProjectTick } from './host';
export type { AuthoredElement, AuthoredTree } from '@diffusionstudio/jsx';
...
export function authoredElement(entity: Entity): AuthoredElement | undefined {
	...
	const text = isText(entity) && !entity.has(Caption) ? entity.get(Chars)?.value : undefined;
 ...
 */
const TRACK_PROPERTIES: Readonly<Record<string, PropertyPath>> = ANIMATABLE_PROPERTIES;
...
export function trackPropertyPath(holder: Entity | null, property: string): PropertyPath | undefined {
  ...
  if (path === 'path.d' && !holder) return undefined;
	if (path === 'width' && holder?.has(Stroke)) return 'stroke.width';
	if (path === 'path.d' && holder?.get(Geometry)?.value === GeometryType.PATH_3D) return 'spatial.path3d';
	return path;
}
...
const MOTION_LIMITS: Readonly<Record<string, readonly [number, number]>> = {
  z: [-Infinity, Infinity], rotationX: [-Infinity, Infinity], rotationY: [-Infinity, Infinity],
  anchorX: [-Infinity, Infinity], anchorY: [-Infinity, Infinity], skewX: [-Infinity, Infinity], skewY: [-Infinity, Infinity],
  cameraX: [-Infinity, Infinity], cameraY: [-Infinity, Infinity], cameraZ: [-Infinity, Infinity],
  cameraRotation: [-Infinity, Infinity], cameraRotationX: [-Infinity, Infinity], cameraRotationY: [-Infinity, Infinity],
  perspective: [Number.MIN_VALUE, Infinity], cameraZoom: [Number.MIN_VALUE, Infinity],
  backdropBlur: [0, Infinity], refraction: [0, 1], bloom: [0, 1], vignette: [0, 1], grain: [0, 1],
  colorSplit: [0, Infinity], focusDistance: [0, Infinity], aperture: [0, Infinity],
};

function validateMotionValue(name: string, value: unknown): void {
  const limits = MOTION_LIMITS[name];
  if (!limits || value === false || value == null) return;
  const number = toNumber(value);
  if (number === undefined || number < limits[0] || number > limits[1]) {
    throw new Error(`${name} must be a finite number between ${limits[0]} and ${limits[1]}`);
  }
}
...
export class RuntimeDocument implements ProjectDocument<SceneNode> {
	...
	public createElement(tag: string): SceneNode {
		...
		switch (name) {
			...
			case 'preset': {
				entity = createEntity(this.world);
				entity.add(Group, Preset, Position);
				break;
			}
			case 'highlight': {
				entity = createEntity(this.world);
				entity.add(Group, Highlight, Position);
				break;
			}
			case 'scene3d': {
                entity = createEntity(this.world);
                entity.add(Group, Geometry, Scene3D, Position, SceneCamera, SpatialParameters, SpatialMaterial);
                resizeEntity(this.world, entity, { width: 1920, height: 1080 });
                break;
            }
            case 'mesh': case 'path3d': case 'pointCloud': case 'light': case 'volume': {
                entity = createEntity(this.world);
                entity.add(Geometry, Position, SpatialGeometry, SpatialParameters, SpatialMaterial);
                const types = {mesh:GeometryType.MESH,path3d:GeometryType.PATH_3D,pointCloud:GeometryType.POINT_CLOUD,light:GeometryType.LIGHT,volume:GeometryType.VOLUME};
                entity.set(Geometry, { value: types[name] });
                if (name === 'path3d') { entity.add(PathData); entity.set(SpatialMaterial, { lit: false }); }
                if (name === 'light') { entity.add(LightSource, Color); entity.set(Color, { value: 0xffffff }); }
                if (name === 'volume') { entity.add(Color); entity.set(Color, { value: 0xffffff }); }
			...
			case 'path': {
				entity = createEntity(this.world);
				entity.add(Geometry);
				entity.set(Geometry, { value: name === 'path' ? GeometryType.PATH : name === 'ellipse' ? GeometryType.ELLIPSE : GeometryType.RECT });
				if (name === 'path') entity.add(PathData);
				entity.add(Position);
				entity.set(Position, { x: 0, y: 0 });
				resizeEntity(this.world, entity, { width: 100, height: 100 });
				break;
	...
	public setProperty(node: SceneNode, name: string, value: unknown): void {
		...
		const { entity } = node;
		if (node.native) validateMotionValue(name, value);
		if (node.native && name === 'depthSort' && value != null && value !== false && value !== 'layer' && value !== 'camera') {
			throw new Error('depthSort must be layer or camera');
		}
		if (entity.has(Keyframe) && name === 'value') {
			const property = getParentEntity(entity)?.get(KeyframeTrack)?.property;
			if (property === 'path.d') value = parsePathData(value);
			else if (property === 'spatial.path3d') value = parsePath3D(value);
			else if (SPATIAL_ARRAYS.some(key => property === `spatial.${key}`)) value = parseNumberArray(value, property!, property!.endsWith('Colors') ? 4 : 3);
            else if (property?.startsWith('spatial.') && Object.hasOwn(SPATIAL_PARAMETERS, property.slice(8))) value = parseSpatialParameter(property.slice(8) as SpatialParameter, value);
			else if (property) validateMotionValue(trackProperty(property) ?? '', value);
		}
		if (entity.has(PathData)) {
			if (name === 'd') value = (entity.get(Geometry)?.value === GeometryType.PATH_3D ? parsePath3D : parsePathData)(value === false || value == null ? '' : value);
			if (name === 'viewBox') value = parsePathViewBox(value === false ? undefined : value);
			if (name === 'fillRule' && value != null && value !== false && value !== 'nonzero' && value !== 'evenodd') throw new Error('Path fillRule must be nonzero or evenodd');
		}
        ...
        if (node.native && name === 'gradientSpace' && value !== 'local' && value !== 'screen' && value != null && value !== false) throw new Error('gradientSpace must be local or screen');
        if (node.native && ['lit','wireframe','castShadow','receiveShadow','depthTest'].includes(name) && value != null && typeof value !== 'boolean') throw new Error(`${name} must be a boolean`);
        if (node.native && ['emissive','fogColor'].includes(name) && value != null && parseColor(value) == null) throw new Error(`${name} must be a color`);
        const physics = node.native && name === 'physics' && value !== false && value != null ? parsePhysicsWorld(value) : null;
        const body = node.native && name === 'rigidBody' && value !== false && value != null ? parseRigidBody(value) : null;
        ...
        if (node.native && Object.hasOwn(SPATIAL_PARAMETERS, name)) {
            entity.add(SpatialParameters); entity.set(SpatialParameters, { [name]: value }); return;
        }
        if (node.native && ['points','pointColors','vertices','vertexColors','indices','normals','uv'].includes(name)) {
            entity.add(SpatialGeometry); entity.set(SpatialGeometry, { [name]: value }); return;
        }
        if (node.native && name === 'shape') { entity.add(SpatialGeometry); entity.set(SpatialGeometry, { shape: value === false || value == null ? 'box' : value as typeof MESH_SHAPES[number] }); return; }
        if (entity.has(LightSource) && name === 'type') { entity.set(LightSource, { type: value as typeof LIGHT_TYPES[number] }); return; }
        if (node.native && ['lit','wireframe','castShadow','receiveShadow','depthTest','emissive','fogColor'].includes(name)) {
            entity.add(SpatialMaterial);
            const fallback = name === 'wireframe' ? false : name === 'emissive' ? '#000000' : name === 'fogColor' ? '#ffffff' : true;
            entity.set(SpatialMaterial, { [name]: value == null ? fallback : value }); return;
        }
        if (node.native && name === 'gradientSpace') { entity.add(GradientSpace); entity.set(GradientSpace, { value: value === 'screen' ? 'screen' : 'local' }); return; }
        if (node.native && name === 'dash') { entity.add(StrokeDash); entity.set(StrokeDash, { value: value as number[] }); return; }
        if (node.native && name === 'physics') { if (!physics) entity.remove(PhysicsWorld); else { entity.add(PhysicsWorld); entity.set(PhysicsWorld, { settings: physics }); } return; }
        if (node.native && name === 'rigidBody') { if (!body) entity.remove(RigidBody); else { entity.add(RigidBody); entity.set(RigidBody, { settings: body }); } return; }
        if (entity.get(Geometry)?.value === GeometryType.PATH_3D && name === 'd') { entity.set(SpatialGeometry, { path: value as string }); return; }

		if (entity.has(Preset) && (name === "preset" || name === "settings")) {
			if (value === false) delete node.props[name];
			entity.set(Preset, parsePresetOptions(node.props));
			return;
		}

		if (entity.has(Highlight) && Object.hasOwn(HIGHLIGHT_DEFAULTS, name)) {
			if (value === false) delete node.props[name];
			entity.set(Highlight, parseHighlightOptions(node.props));
			return;
		}
		...
		switch (name) {
			...
			case 'link': {
				if (typeof value !== 'string' || !value) {
					entity.remove(ClipLink);
					return;
				}
				entity.add(ClipLink);
				entity.set(ClipLink, { value });
				return;
			}
			case 'name': {
				if (typeof value !== 'string' || !value) {
			...
			case 'd': {
				if (!entity.has(PathData)) return;
				entity.set(PathData, { value: typeof value === 'string' ? value : '' });
				return;
			}
			case 'fillRule': {
				if (entity.has(PathData)) entity.set(PathData, { fillRule: value === 'evenodd' ? 'evenodd' : 'nonzero' });
				return;
			}
			case 'viewBox': {
				if (!entity.has(PathData)) return;
				entity.set(PathData, { viewBox: parsePathViewBox(value) });
				return;
			}
			case 'anchorX':
			case 'anchorY': {
				entity.add(Anchor);
				entity.set(Anchor, { [name === 'anchorX' ? 'x' : 'y']: toNumber(value) ?? 0.5 });
				return;
			}
			case 'skewX':
			case 'skewY': {
				entity.add(Skew);
				entity.set(Skew, { [name === 'skewX' ? 'x' : 'y']: toNumber(value) ?? 0 });
				return;
			}
			case 'flipX':
			case 'flipY': {
				entity.add(Flip);
				entity.set(Flip, { [name === 'flipX' ? 'x' : 'y']: value === true ? -1 : 1 });
				return;
			}
			case 'rotationX':
			case 'rotationY': {
				entity.add(Rotation);
				entity.set(Rotation, { [name === 'rotationX' ? 'x' : 'y']: toNumber(value) ?? 0 });
				return;
			}
			case 'depthSort': {
				if (!entity.has(Group) && !entity.has(Scene)) return;
				if (value === 'camera') {
					entity.add(DepthSort);
					entity.set(DepthSort, { value: 'camera' });
				} else entity.remove(DepthSort);
				return;
			}
			case 'backdropBlur':
			case 'refraction': {
				entity.add(LayerMaterial);
				entity.set(LayerMaterial, { [name]: toNumber(value) ?? 0 });
				return;
			}
			case 'bloom':
			case 'vignette':
			...
			case 'focusDistance':
			case 'aperture': {
				if (!entity.has(Scene) && !entity.has(Scene3D)) return;
				entity.add(SceneEffects);
				entity.set(SceneEffects, { [name]: toNumber(value) ?? (name === 'focusDistance' ? 1000 : 0) });
				return;
			}
			...
			case 'cameraRotation': {
				if (!entity.has(Scene) && !entity.has(Scene3D)) return;
				const number = toNumber(value);
				const fallback = name === 'cameraX' || name === 'cameraY' ? null : name === 'perspective' || name === 'cameraZ' ? 1000 : name === 'cameraZoom' ? 1 : 0;
				entity.add(SceneCamera);
				entity.set(SceneCamera, { [name]: number ?? fallback });
				return;
			}
			...
			case 'transition': {
				...
				if ('duration' in spec) {
					entity.set(Transition, { duration: this.toFrames(this.toSeconds(spec.duration) ?? 1) });
				}
				return;
			}
			case 'wgsl': {
			...
			case 'height': {
				...
				if (size === undefined) {
					// Text and groups resume intrinsic sizing when neither
					// bound is authored. Size holds both axes.
					const other = name === 'width' ? 'height' : 'width';
					if ((isText(entity) || entity.has(Group)) && toNumber(node.props[other]) === undefined) {
						entity.remove(Size);
					}
					return;
				}
			...
			case 'audioStream': {
				if (value === undefined || value === null || value === false) {
					entity.remove(AudioStream);
					return;
				}
				if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) throw new Error('Audio stream must be a nonnegative integer');
				entity.add(AudioStream);
				entity.set(AudioStream, { value });
				return;
			}
			case 'colorGrade': {
				const settings = parseColorGrade(value);
				if (!Object.keys(settings).length) entity.remove(ColorGrade);
				else {
					entity.add(ColorGrade);
					entity.set(ColorGrade, { value: settings });
				}
				return;
			}
			case 'audioProcessing': {
				const settings = parseAudioProcessing(value);
				if (!Object.keys(settings).length) entity.remove(AudioProcessing);
				else {
					entity.add(AudioProcessing);
					entity.set(AudioProcessing, { value: settings });
				}
				return;
			}
			case 'muted': {
			...
			case 'markers': {
				...
				const markers = value.map((marker: unknown) => {
					if (!marker || typeof marker !== 'object' || !('id' in marker) || typeof marker.id !== 'string' || !marker.id
						|| !('name' in marker) || typeof marker.name !== 'string' || !('time' in marker)) throw new Error('Each marker needs an id, name, and time');
					const seconds = this.toSeconds(marker.time);
					if (seconds === undefined || seconds < 0 || ids.has(marker.id)) throw new Error('Markers require unique ids and nonnegative times');
					ids.add(marker.id);
					return { id: marker.id, time: this.toFrames(seconds), name: marker.name };
				});
				entity.add(Markers);
				entity.set(Markers, { value: markers });
				return;
			}
			case 'workarea': {
				// Two times or none: a half-authored range says nothing, and
				// `false` is what the editor writes to take the brackets off.
				const range = Array.isArray(value) ? value.map((time) => this.toSeconds(time)) : [];
	...
	private toSeconds(value: unknown): number | undefined {
		if (typeof value !== 'number' && typeof value !== 'string') return undefined;
		return parseTime(value, this.world.get(ProjectFrameRate)?.value ?? this.world.get(FrameRate)?.value ?? 30);
	}
	...
	private syncTiming(node: SceneNode): void {
		...
		const start = this.toSeconds(props.start);
		const end = this.toSeconds(props.end);
		const sourceIn = this.toSeconds(props.sourceIn);
		const sourceOut = this.toSeconds(props.sourceOut);
	...
	private resolveTrackProperty(track: Entity, property: unknown): void {
		...
		for (const child of track.get(Host)?.children ?? []) {
			if (child.entity.has(Keyframe)) this.syncKeyframeValue(child);
		}
	}

	private syncKeyframeValue(node: SceneNode): void {
		const value = node.props.value;
		const property = getParentEntity(node.entity)?.get(KeyframeTrack)?.property;
		if (property) validateMotionValue(trackProperty(property) ?? '', value);
		if (property === 'path.d' || property === 'spatial.path3d') {
        ...
        if (property?.startsWith('spatial.') && Object.hasOwn(SPATIAL_PARAMETERS, property.slice(8))) {
            node.entity.set(Keyframe, { value: parseSpatialParameter(property.slice(8) as SpatialParameter, value), stringValue: undefined, arrayValue: undefined }); return;
        }
        node.entity.set(Keyframe, { value: toNumber(value) ?? parseColor(value) ?? 0, stringValue: undefined, arrayValue: undefined });
	}
	...
	public insertNode(parent: SceneNode, node: SceneNode, anchor?: SceneNode): void {
		...
		if (
			...
			isAncestorOf(node.entity, parent.entity)
		) {
		...
		if (getParentEntity(node.entity) !== parent.entity) {
			...
			if (node.entity.has(Keyframe)) this.syncKeyframeValue(node);
		}

		let index = 0;
		for (const sibling of parent.children) {
			if (!sibling.native) continue;
			if (sibling.entity.get(ItemIndex)?.value !== index) {
