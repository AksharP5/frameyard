...
export const TIME_FORMS = `seconds ("1.5"), frames ("45f"), or "MM:SS" / "HH:MM:SS" / "HH:MM:SS:FF"`;
...
export const Time = z
  ...
  .transform((value, ctx) => {
    const clock = typeof value === "string" ? value.trim().replace(/^-/, "") : "";
    if (clock.split(":").length === 4 && !/^\d{2}:\d{2}:\d{2}:\d{2}$/.test(clock)) {
