...
export type Mat2D = {
	...
	/** Optional projective row, used by spatial layers. Ordinary 2D stays affine. */
...
export function multiply2D(p: Mat2D, q: Mat2D): Mat2D {
	if (p.i !== undefined || q.i !== undefined) {
		const pg = p.g ?? 0, ph = p.h ?? 0, pi = p.i ?? 1;
		const qg = q.g ?? 0, qh = q.h ?? 0, qi = q.i ?? 1;
		return {
			a: p.a * q.a + p.c * q.b + p.e * qg,
			b: p.b * q.a + p.d * q.b + p.f * qg,
			c: p.a * q.c + p.c * q.d + p.e * qh,
			d: p.b * q.c + p.d * q.d + p.f * qh,
			e: p.a * q.e + p.c * q.f + p.e * qi,
			f: p.b * q.e + p.d * q.f + p.f * qi,
			g: pg * q.a + ph * q.b + pi * qg,
			h: pg * q.c + ph * q.d + pi * qh,
			i: pg * q.e + ph * q.f + pi * qi,
		};
	}
	return {
		a: p.a * q.a + p.c * q.b,
