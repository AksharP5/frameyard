...
export function aabbFromTransformedRect(mat: Mat2D, w: number, h: number): AABB {
	if (mat.i !== undefined) {
		const points = rectToQuad(mat, w, h);
		return {
...
export function rectToQuad(mat: Mat2D, w: number, h: number): Quad {
	if (mat.i !== undefined) {
		return [transformPoint(mat, 0, 0), transformPoint(mat, w, 0), transformPoint(mat, w, h), transformPoint(mat, 0, h)];
	}
	const { a, b, c, d, e, f } = mat;
	return [
		{ x: e, y: f },
...
export function transformPoint(mat: Mat2D, px: number, py: number): Point {
	const denominator = mat.i === undefined ? 1 : (mat.g ?? 0) * px + (mat.h ?? 0) * py + mat.i;
	return {
		x: (mat.a * px + mat.c * py + mat.e) / denominator,
		y: (mat.b * px + mat.d * py + mat.f) / denominator,
	};
}
...
export function invert2D(m: Mat2D): Mat2D {
	const { a, b, c, d, e, f } = m;
	if (m.i !== undefined) {
		const g = m.g ?? 0, h = m.h ?? 0, i = m.i;
		const determinant = a * (d * i - f * h) - c * (b * i - f * g) + e * (b * h - d * g);
		return {
			a: (d * i - f * h) / determinant, b: (f * g - b * i) / determinant,
			c: (e * h - c * i) / determinant, d: (a * i - e * g) / determinant,
			e: (c * f - e * d) / determinant, f: (e * b - a * f) / determinant,
			g: (b * h - d * g) / determinant, h: (c * g - a * h) / determinant,
			i: (a * d - c * b) / determinant,
		};
	}
	const det = a * d - b * c;
