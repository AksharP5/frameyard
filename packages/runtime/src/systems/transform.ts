...
import { computeSpatialTransforms } from './spatial';
...
export function computeLocalMatrix(world: World, entity: Entity): void {
	...
	const pivotX = (computed.anchorX[eid] ?? anchor.x[eid] ?? 0.5) * computed.width[eid];
...
export function computeGroupBounds(world: World, entity: Entity): void {
	...
	if (entity.has(Sequential) || entity.has(Highlight) || entity.has(Preset)) {
	...
	if (entity.has(Group)) {
		if (entity.has(Size)) {
			computed.originX[eid] = 0;
			computed.originY[eid] = 0;
			return;
		}
		let minX = Infinity;
		let minY = Infinity;
...
function adjustLayers(world: World, adjust: Entity): void {
	...
	const reworld = (entity: Entity, parentEntity: Entity | null) => {
		...
		for (const child of getNodeChildren(world, entity)) {
			if (!child.has(Geometry) && !child.has(Group)) continue;
...
export function transformSystem(world: World): void {
	physicsSystem(world);
	if (world.get(RenderSurface)?.ctx) {
		for (const entity of world.query(Geometry, Not(Hidden))) {
			if (entity.get(Geometry)?.value === GeometryType.TEXT) layoutText(world, entity);
		}
	}
	...
	computeSpatialTransforms(world);
}
