...
// Clips with the same token are edited together without sharing their source.
export const ClipLink = trait({ value: '' });

export const Locked = trait();
export type TimelineMarker = { id: string; time: number; name: string };
export const Markers = trait({ value: () => [] as TimelineMarker[] });
