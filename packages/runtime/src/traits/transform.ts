...
export const Rotation = trait({ value: 0, x: 0, y: 0 });

export const Scale = trait({ x: 1, y: 1 });

export const UniformScale = trait({ value: 1 });

export const Anchor = trait({ x: 0.5, y: 0.5 });
...
// Scene-local camera, separate from the editor's infinite-canvas viewport.
export const SceneCamera = trait({
  perspective: 1000,
  cameraX: null as number | null,
  cameraY: null as number | null,
  cameraZ: 1000,
  cameraZoom: 1,
  cameraRotationX: 0,
  cameraRotationY: 0,
