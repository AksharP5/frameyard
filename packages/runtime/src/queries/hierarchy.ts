...
import { Cache, ChildOf, Computed, Name, Playback, Geometry, Group, AdjustmentLayer, Paint } from '../traits';
...
/** Whether `ancestor` is above `entity` in the runtime tree, including the stage. */
export function isAncestorOf(ancestor: Entity, entity: Entity): boolean {
	for (let parent = getParentEntity(entity); parent !== null; parent = getParentEntity(parent)) {
...
export function getNodePaints(world: World, parent: Entity): readonly Entity[] {
	if (!parent.has(Cache) || isStage(parent)) return world.query(ChildOf(parent), Paint, Not(Geometry));
	const cache = store(world, Cache);
	const id = parent.id();
	const fills = cache.fills[id]!;
	const strokes = cache.strokes[id]!;
	return strokes.length === 0 ? fills : fills.concat(strokes.filter(stroke => stroke.has(Paint)));
}
