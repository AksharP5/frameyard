...
const cachedLayers = new WeakMap<World, Map<Entity, TimelineNode[]>>();

/** Shared row structure. Playback and selection do not change the tree. */
export function buildTimelineLayers(world: World, parent: Entity): TimelineNode[] {
	let cache = cachedLayers.get(world);
	if (!cache) {
		cache = new Map();
		...
		world.onChange(ItemIndex, invalidate);
	}
	let layers = cache.get(parent);
	if (!layers) {
		layers = buildLayers(world, parent);
		cache.set(parent, layers);
	}
	return layers;
}

function buildLayers(world: World, parent: Entity): TimelineNode[] {
...
function buildNode(world: World, entity: Entity, kind: TimelineNodeKind): TimelineNode {
	...
	const children = buildLayers(world, entity);
	return {
		entity,
		kind,
		expanded: true,
