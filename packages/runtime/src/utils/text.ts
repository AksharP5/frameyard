...
import {
	...
	Chars, TextStyle, TextRange, TextCache, Cache, Computed,
	RenderSurface,
} from '../traits';
...
function tokenizeText(world: World, entity: Entity) {
	...
	const maxWidth = entity.has(Size) ? store(world, Size).width[entity.id()] ?? Number.POSITIVE_INFINITY : Number.POSITIVE_INFINITY;
	...
	// Draw all text shadows
	{
		...
		for (const word of words) {
			...
			for (const shadow of shadows) {
				...
				ctx.shadowOffsetX = (computed.offsetX[sid] ?? 0) * shadowScale;
				ctx.shadowOffsetY = (computed.offsetY[sid] ?? 0) * shadowScale;
				ctx.shadowBlur = (computed.blur[sid] ?? 0) * shadowScale;
				ctx.shadowColor = colorToHex(computed.color[sid] ?? 0x000000);
				ctx.fillStyle = colorToHex(computed.color[sid] ?? 0x000000);
				ctx.globalAlpha = savedAlpha * (computed.opacity[sid] ?? 1);
...
export function renderText(world: World, entity: Entity, prepare = true) {
	const ctx = world.get(RenderSurface)?.ctx;
	if (!ctx) return;
	if (prepare) layoutText(world, entity);
