...
import { StrokeStyle, Computed, Hidden, StrokeDash } from '../traits';
...
export function applyStrokeStyle(ctx: Ctx, world: World, stroke: Entity): void {
	...
	ctx.setLineDash(stroke.get(StrokeDash)?.value ?? []);
	ctx.lineDashOffset = store(world, Computed).dashOffset[sid] ?? 0;
}
