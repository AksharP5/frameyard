...
import { isAncestorOf, getParentNode } from '../queries/hierarchy';
...
export function appendChild(_world: World, entity: Entity, parent: Entity): void {
	assert(getParentNode(entity) === null, 'Entity already has a parent');
	assert(parent !== entity, 'Cannot parent entity into itself');
	assert(!isAncestorOf(entity, parent), 'Cannot parent entity into its own subtree');
