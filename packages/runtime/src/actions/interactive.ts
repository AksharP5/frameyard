...
export function findChildAt(world: World, parent: Entity, point: Point): Entity | null {
	const children = world.query(Or(Geometry, Group, AdjustmentLayer), ChildOf(parent), Not(IsMask), Not(Culled));

	for (let i = children.length - 1; i >= 0; i--) {
		const child = children[i]!;
		if (child.has(Sequential)) {
			const nested = findChildAt(world, child, point);
...
export function enterEntity(world: World, parent: Entity, point: Point): Entity | null {
	...
	const expose = (container: Entity): void => {
		for (const sibling of world.query(Or(Geometry, Group, AdjustmentLayer), ChildOf(container))) {
			if (sibling.has(Sequential)) expose(sibling);
			else sibling.add(Interactive);
		}
	};
	expose(parent);
