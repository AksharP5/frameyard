...
import {
	Audio, AudioEngine, AudioDecoderHandle, AudioPlayback, Cache, ChildOf, Computed, FrameRate, Geometry, Paint,
	Playback, Scene, Stage, Workarea, Mode, RenderSurface,
} from '../traits';
 ...
 * is how it gets longer. A seek stops previously scheduled sound and anchors
 * continuing playback to the new position on the audio clock.
 */
export function setPlayhead(world: World, scene: Entity, frame: number): void {
	...
	if (surface?.canvas && world.get(Mode)?.value === 'realtime') {
		world.set(RenderSurface, { pendingSeek: {
			scene, width: surface.canvas.width, height: surface.canvas.height, view: getViewMatrix(world),
		} });
	}

	if (scene.get(Playback)?.playing) {
		if (!scene.has(AudioPlayback)) scene.add(AudioPlayback);
		scene.set(AudioPlayback, {
			wasPlaying: false,
			contextOffsetInSeconds: world.get(AudioEngine)?.context?.currentTime ?? 0,
			timelineOffsetInSeconds: clamped / fps,
		});
	}
	const pending = [scene];
	for (const entity of pending) {
		entity.get(AudioDecoderHandle)?.reset();
		pending.push(...world.query(ChildOf(entity)));
	}
}
...
export function togglePlayback(world: World, scene: Entity): void {
	...
	// Starting outside the playback range would stop on the first advance.
	if (!playback.playing) {
		const computed = store(world, Computed);
		const eid = scene.id();
		const workarea = scene.has(Workarea) ? scene.get(Workarea) : undefined;
		...
		const start = workarea?.start ?? 0;
		const frame = computed.localTime[eid] ?? 0;
		if (end > 0 && (frame < start || frame >= end)) {
