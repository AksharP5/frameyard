...
import { readMediaFile } from '@diffusionstudio/assets';
...
/**
 * Returns the asset's decodable media, reusing an in-flight or resolved read.
 * Its original remains available through handle.getFile().
 */
export function getAssetFile(asset: Pick<Asset, 'handle'>): Promise<File> {
	...
	const promise = readMediaFile(asset);
