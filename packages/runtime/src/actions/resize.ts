...
function resolveConstraintOffsetsAgainst(world: World, entity: Entity, dims: Entity): void {
	...
	// Relation-only queries read the child index without creating a permanent
	// filtered subscription for every entity visited during project loading.
	const children = world.query(ChildOf(entity));
	// Sequential children resolve against the same spatial parent.
	for (const child of children) {
		if (child.has(Sequential)) resolveConstraintOffsetsAgainst(world, child, dims);
	}

	for (const child of children) {
		if (child.has(Sequential) || !child.has(Constraint)) continue;
