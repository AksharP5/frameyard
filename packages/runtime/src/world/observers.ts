...
import {
	...
	bubbleTimeRangeUp, fitsChildren,
} from '../actions/timing';
...
export function observeWorld(world: World): () => void {
	...
	subs.push(world.onRemove(ChildOf('*'), (child, parent) => {
		...
		// Destroy may strip the child's type before detaching it. Refit the
		// known parent after eviction even when the child can no longer bubble.
		if (fitsChildren(parent)) {
			recomputeEntityTimeRange(world, parent);
			bubbleTimeRangeUp(world, parent);
		}
		syncStagePlayback(parent);
	}));
	...
	// Destroy strips the track trait before detaching its relation. Refresh
	...
	mirror(Position, (entity) => {
		...
		computed.positionZ[entity.id()] = z;
	...
	mirror(Rotation, (entity) => {
		const computed = store(world, Computed);
		const { value, x, y } = entity.get(Rotation)!;
		computed.rotation[entity.id()] = value;
		computed.rotationX[entity.id()] = x;
		computed.rotationY[entity.id()] = y;
	});

	mirror(Anchor, (entity) => {
		const computed = store(world, Computed);
		const { x, y } = entity.get(Anchor)!;
		computed.anchorX[entity.id()] = x;
		computed.anchorY[entity.id()] = y;
	});

	mirror(PathData, (entity) => {
		store(world, Computed).pathData[entity.id()] = entity.get(PathData)!.value;
	});

	mirror(SceneCamera, (entity) => resetAnimatedValues(world, entity));
	mirror(SpatialParameters, (entity) => resetAnimatedValues(world, entity));
	mirror(SpatialGeometry, (entity) => resetAnimatedValues(world, entity));
	mirror(LayerMaterial, (entity) => resetAnimatedValues(world, entity));
	mirror(SceneEffects, (entity) => resetAnimatedValues(world, entity));
	...
	mirror(Size, (entity) => {
		...
		if (entity.has(Scene) || entity.has(Scene3D)) {
			const camera = entity.get(SceneCamera);
			const size = entity.get(Size)!;
			const computed = store(world, Computed);
			computed.cameraX[entity.id()] = camera?.cameraX ?? size.width / 2;
			computed.cameraY[entity.id()] = camera?.cameraY ?? size.height / 2;
		}
	}, false);
