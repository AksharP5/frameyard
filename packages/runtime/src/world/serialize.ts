import { Scene3D, SpatialParameters, SpatialGeometry, SpatialMaterial, LightSource, GradientSpace, StrokeDash, PhysicsWorld, RigidBody } from '../traits';
...
import {
	...
	Volume, Muted, AudioProcessing, AudioStream,
	KeyframeTrack, Keyframe, Animation, Stage,
} from '../traits';
import { createEntity } from '../actions/entities';

import type { Entity, World, TraitRecord } from 'koota';
import { parseHighlightOptions, parsePresetOptions, type PresetOptions, type HighlightOptions, type AudioProcessingSettings, type ColorGradeSettings } from '@diffusionstudio/jsx';
...
export interface EntityRecord {
	Scene3D?: {};
	SpatialParameters?: TraitRecord<typeof SpatialParameters>;
	SpatialGeometry?: TraitRecord<typeof SpatialGeometry>;
	SpatialMaterial?: TraitRecord<typeof SpatialMaterial>;
	LightSource?: TraitRecord<typeof LightSource>;
	GradientSpace?: TraitRecord<typeof GradientSpace>;
	StrokeDash?: TraitRecord<typeof StrokeDash>;
	PhysicsWorld?: TraitRecord<typeof PhysicsWorld>;
	RigidBody?: TraitRecord<typeof RigidBody>;

	id: string;
	eid: number;
	Geometry?: number;
	PathData?: TraitRecord<typeof PathData>;
	SceneCamera?: TraitRecord<typeof SceneCamera>;
	LayerMaterial?: TraitRecord<typeof LayerMaterial>;
	SceneEffects?: TraitRecord<typeof SceneEffects>;
	Paint?: number;
	Group?: {};
	Scene?: {};
	DepthSort?: TraitRecord<typeof DepthSort>;
	Audio?: {};
	AdjustmentLayer?: {};
	...
	ClipHeight?: number;
	Expanded?: {};
	Position?: Partial<TraitRecord<typeof Position>>;
	Offset?: {
		x?: number;
		y?: number;
	};
	...
	Keyframe?: Partial<TraitRecord<typeof Keyframe>>;
	Animation?: {
		duration?: number;
		delay?: number;
		type?: number;
...
export function serializeEntity(entity: Entity): EntityRecord {
	...
	if (entity.has(Preset)) {
		record.Preset = parsePresetOptions(entity.get(Preset));
	}
	if (entity.has(Highlight)) {
		record.Highlight = parseHighlightOptions(entity.get(Highlight));
	}
	...
	if (entity.has(DepthSort)) record.DepthSort = { ...entity.get(DepthSort)! };
	if (entity.has(PathData)) {
		const path = entity.get(PathData)!;
		record.PathData = { value: path.value, fillRule: path.fillRule, viewBox: path.viewBox ? [...path.viewBox] : undefined };
	}
	if (entity.has(SceneCamera)) record.SceneCamera = { ...entity.get(SceneCamera)! };
	if (entity.has(LayerMaterial)) record.LayerMaterial = { ...entity.get(LayerMaterial)! };
	if (entity.has(SceneEffects)) record.SceneEffects = { ...entity.get(SceneEffects)! };
	if (entity.has(Scale)) {
	...
	if (entity.has(AudioProcessing)) {
		record.AudioProcessing = structuredClone(entity.get(AudioProcessing)!.value);
	}
	...
	if (entity.has(ColorGrade)) record.ColorGrade = structuredClone(entity.get(ColorGrade)!.value);
	if (entity.has(AudioStream)) record.AudioStream = entity.get(AudioStream)!.value;
	if (entity.has(Locked)) record.Locked = true;
	if (entity.has(Markers)) record.Markers = structuredClone(entity.get(Markers)!.value);
	if (entity.has(ClipLink)) {
		record.ClipLink = entity.get(ClipLink)!.value;
	}
	...
	if (entity.has(Scene3D)) record.Scene3D = {};
	if (entity.has(SpatialParameters)) record.SpatialParameters = structuredClone(entity.get(SpatialParameters)!);
	if (entity.has(SpatialGeometry)) record.SpatialGeometry = structuredClone(entity.get(SpatialGeometry)!);
	if (entity.has(SpatialMaterial)) record.SpatialMaterial = structuredClone(entity.get(SpatialMaterial)!);
	if (entity.has(LightSource)) record.LightSource = structuredClone(entity.get(LightSource)!);
	if (entity.has(GradientSpace)) record.GradientSpace = structuredClone(entity.get(GradientSpace)!);
	if (entity.has(StrokeDash)) record.StrokeDash = structuredClone(entity.get(StrokeDash)!);
	if (entity.has(PhysicsWorld)) record.PhysicsWorld = structuredClone(entity.get(PhysicsWorld)!);
	if (entity.has(RigidBody)) record.RigidBody = structuredClone(entity.get(RigidBody)!);
	return record;
}
...
export function deserializeEntity(entity: Entity, e: Partial<EntityRecord>): void {
	if (e.Scene3D) entity.add(Scene3D);
	if (e.SpatialParameters) { entity.add(SpatialParameters); entity.set(SpatialParameters, structuredClone(e.SpatialParameters)); }
	if (e.SpatialGeometry) { entity.add(SpatialGeometry); entity.set(SpatialGeometry, structuredClone(e.SpatialGeometry)); }
	if (e.SpatialMaterial) { entity.add(SpatialMaterial); entity.set(SpatialMaterial, structuredClone(e.SpatialMaterial)); }
	if (e.LightSource) { entity.add(LightSource); entity.set(LightSource, structuredClone(e.LightSource)); }
	if (e.GradientSpace) { entity.add(GradientSpace); entity.set(GradientSpace, structuredClone(e.GradientSpace)); }
	if (e.StrokeDash) { entity.add(StrokeDash); entity.set(StrokeDash, structuredClone(e.StrokeDash)); }
	if (e.PhysicsWorld) { entity.add(PhysicsWorld); entity.set(PhysicsWorld, structuredClone(e.PhysicsWorld)); }
	if (e.RigidBody) { entity.add(RigidBody); entity.set(RigidBody, structuredClone(e.RigidBody)); }
	...
	if (e.Preset !== undefined) {
		entity.add(Preset);
		entity.set(Preset, parsePresetOptions(e.Preset));
	}
	if (e.Highlight !== undefined) {
		entity.add(Highlight);
		entity.set(Highlight, parseHighlightOptions(e.Highlight));
	}
	...
	if (e.RotationX !== undefined || e.RotationY !== undefined) {
		entity.add(Rotation);
		entity.set(Rotation, { x: e.RotationX ?? 0, y: e.RotationY ?? 0 });
	}
	if (e.DepthSort !== undefined) {
		entity.add(DepthSort);
		entity.set(DepthSort, e.DepthSort);
	}
	if (e.PathData !== undefined) {
		entity.add(PathData);
		entity.set(PathData, { value: e.PathData.value, fillRule: e.PathData.fillRule, viewBox: e.PathData.viewBox ? [...e.PathData.viewBox] : undefined });
	}
	if (e.LayerMaterial !== undefined) {
		entity.add(LayerMaterial);
		entity.set(LayerMaterial, e.LayerMaterial);
	}
	if (e.SceneEffects !== undefined) {
		entity.add(SceneEffects);
		entity.set(SceneEffects, e.SceneEffects);
	}
	if (e.SceneCamera !== undefined) {
		entity.add(SceneCamera);
		entity.set(SceneCamera, e.SceneCamera);
	}
	...
	if (e.AudioProcessing !== undefined) {
		entity.add(AudioProcessing({ value: structuredClone(e.AudioProcessing) }));
	}
	...
	if (e.ColorGrade) entity.add(ColorGrade({ value: structuredClone(e.ColorGrade) }));
	if (e.AudioStream !== undefined) entity.add(AudioStream({ value: e.AudioStream }));
	if (e.Locked) entity.add(Locked);
	if (e.Markers) entity.add(Markers({ value: structuredClone(e.Markers) }));
	if (e.ClipLink !== undefined) {
		entity.add(ClipLink({ value: e.ClipLink }));
	}
