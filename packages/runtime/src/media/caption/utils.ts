...
import { ChildOf, TextRange, Cache, Chars, FrameRate, Trim } from '../../traits';
...
const trimmedGroups = new WeakMap<WordGroup, { start: number; end: number; words: WordGroup }>();

/** Keep only words heard in this clip, retaining the preset's group boundaries. */
export function findActiveCaptionGroup(world: World, entity: Entity, groups: WordGroup[], relativeTime: number): WordGroup | undefined {
	const group = groups[findActiveGroup(groups, relativeTime)];
	if (!group) return undefined;
	const source = getSourceWindow(entity);
	const fps = world.get(FrameRate)?.value ?? 30;
	const start = source.in / fps;
	// Rounded timeline durations can extend a fraction past the source trim at faster rates.
	const end = Math.min(source.out, entity.get(Trim)?.end ?? Infinity) / fps;
	let cached = trimmedGroups.get(group);
	if (!cached || cached.start !== start || cached.end !== end) {
		cached = { start, end, words: group.filter(word => word.end > start && word.start < end) };
		trimmedGroups.set(group, cached);
	}
	return cached.words.length ? cached.words : undefined;
}
