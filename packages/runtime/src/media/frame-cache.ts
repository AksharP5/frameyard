...
export class FrameCache {
  ...
  private evictTiles() {
    ...
    this.tiles = this.tiles.filter(t => t.endFrameIndex > this.leftFrameIndex && t.frameIndex <= this.rightFrameIndex);
  }

  private insertTile(frame: Frame, frameIndex: number, endFrameIndex: number) {
    let tileIndex = 0;
    ...
    ctx.drawImage(this.rotation ? this.source : frame, x, y, this.tileWidth, this.tileHeight);
  }
  ...
  public hasAtOrAfter(frameIndex: number) {
    return this.tiles.some(t => t.frameIndex >= frameIndex);
  }

  public findCovering(frameIndex: number) {
    let latest: FrameTile | undefined;
    for (const tile of this.tiles) {
      if (tile.frameIndex <= frameIndex && (!latest || tile.frameIndex > latest.frameIndex)) latest = tile;
    }
    return latest && frameIndex < latest.endFrameIndex ? latest.frameIndex : undefined;
  }

  public findAtOrBefore(frameIndex: number) {
    let previous: number | undefined;
    for (const tile of this.tiles) {
      if (tile.frameIndex <= frameIndex && (previous === undefined || tile.frameIndex > previous)) previous = tile.frameIndex;
    }
    return previous;
  }
  ...
  public insert(frame: VideoFrame | ImageBitmap, index: number, endFrameIndex = index + 1) {
    if (this.has(index)) return;

    this.evictTiles();
    this.resizeCaches(frame);
    // Most footage can downscale straight into its tile without a full-resolution copy.
    if (this.rotation) this.drawRotated(frame);
    this.insertTile(frame, index, endFrameIndex);
